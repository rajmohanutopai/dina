 ...
 * grow to match.
 */
export interface PeerlensSearchFilters {
  /**
...
export interface PeerlensSearchQuery extends PeerlensSearchFilters {
...
export interface PeerlensReview {
  reviewerDID: string;
  reviewerName?: string;
...
export interface PeerlensSearchResult {
  query: string;
  type: SearchType;
  reviews: PeerlensReview[];
...
let queryClient: PeerlensQueryClient | null = null;

/** Register the AppView trust query client. */
export function registerTrustQueryClient(client: PeerlensQueryClient): void {
  queryClient = client;
}
...
const searchCache = new Map<string, { result: PeerlensSearchResult; cachedAt: number }>();

function getCachedSearch(key: string, now?: number): PeerlensSearchResult | null {
...
function cacheSearch(key: string, result: PeerlensSearchResult): void {
 ...
 * format evolves.
 */
export function dropSearchCache(query: PeerlensSearchQuery): void {
 ...
 */
export async function searchTrustNetwork(query: PeerlensSearchQuery): Promise<PeerlensSearchResult> {
  ...
  const result: PeerlensSearchResult = {
    query: query.query,
    type: query.type,
    reviews: limited,
    aggregateScore,
 ...
 */
function searchLocalContacts(query: PeerlensSearchQuery): PeerlensReview[] {
  const reviews: PeerlensReview[] = [];
 ...
 */
function profileToReviews(profile: PeerlensProfile): PeerlensReview[] {
 ...
 *   - Network attestations: weight 0.5
 */
function computeWeightedAggregate(reviews: PeerlensReview[]): number | null {
...
/**
 * Pull just the `PeerlensSearchFilters` keys out of the wider
 * `PeerlensSearchQuery`. Used so we can fingerprint or AppView-call
 * with the filters without dragging the type/query/limit fields.
 */
function extractFilters(q: PeerlensSearchQuery): PeerlensSearchFilters {
  return {
...
function hasAnyFilter(f: PeerlensSearchFilters): boolean {
  return (
...
/**
 * Convert one AppView search hit into a `PeerlensReview` for the
 ...
 */
function searchHitToReview(hit: AttestationSearchHit): PeerlensReview {
 ...
 */
function buildCacheKey(query: PeerlensSearchQuery): string {
  const filters = extractFilters(query);
  const limit = query.limit ?? 20;

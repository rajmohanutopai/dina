 ...
 */
export interface PeerlensProfile {
  did: string;
  /**
...
export class PeerlensQueryClient {
   ...
   */
  toTrustScore(profile: PeerlensProfile): TrustScore {
    return {
      did: profile.did,
...
/**
 * Parse a raw xRPC response into a PeerlensProfile.
 */
function parseProfile(data: Record<string, unknown>): PeerlensProfile {
  return {
    did: String(data.did ?? ''),

...
import { PEOPLE_APPLY_EXTRACTION, PEOPLE_FIND, PEOPLE_LIST } from './paths';
...
export function registerPeopleRoutes(router: CoreRouter, options: PeopleRouteOptions = {}): void {
  const { applyExtraction, list, find } = makePeopleHandlers(options);
  router.post(PEOPLE_APPLY_EXTRACTION, applyExtraction);
  // /find must be registered before /list-style prefix matches; CoreRouter
  // uses exact path matching so both work, but listing /find first keeps
  // intent obvious.

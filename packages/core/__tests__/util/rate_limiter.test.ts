...
import { RateLimiter, type RateLimiterOptions } from '../../src/util/rate_limiter';
...
describe('RateLimiter — construction', () => {
  it.each([
    ...
    expect(() => new RateLimiter(bad as unknown as RateLimiterOptions)).toThrow();
  });

  it('rejects initialTokens out of range', () => {
    expect(() => new RateLimiter({ capacity: 10, refillPerSec: 1, initialTokens: -1 })).toThrow(
      /initialTokens/,
    );
    expect(() => new RateLimiter({ capacity: 10, refillPerSec: 1, initialTokens: 11 })).toThrow(
      /initialTokens/,
    );
  });

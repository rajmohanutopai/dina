...
describe('PeerlensQueryClient (9.1)', () => {
  describe('queryProfile', () => {
    it('fetches PeerLens profile from AppView xRPC', async () => {
      ...
      const client = new PeerlensQueryClient({ fetch: mockFetch });
    ...
    it('returns network error on fetch failure', async () => {
      ...
      const client = new PeerlensQueryClient({ fetch: failFetch });
    ...
    it('returns timeout error on abort', async () => {
      ...
      const client = new PeerlensQueryClient({ fetch: timeoutFetch });
    ...
    it('returns error for empty DID', async () => {
      const client = new PeerlensQueryClient({ fetch: createMockFetch({}).mockFetch });
    ...
    it('clamps overallTrustScore to [0, 1]', async () => {
      ...
      const high = await new PeerlensQueryClient({ fetch: highMock }).queryProfile('did:plc:test');
      expect(high.profile!.overallTrustScore).toBe(1);

      const { mockFetch: lowMock } = createMockFetch({ ...SAMPLE_PROFILE, overallTrustScore: -0.4 });
      const low = await new PeerlensQueryClient({ fetch: lowMock }).queryProfile('did:plc:test');
    ...
    it('uses custom AppView URL', async () => {
      const { mockFetch, calls } = createMockFetch(SAMPLE_PROFILE);
      const client = new PeerlensQueryClient({
  ...
  describe('toTrustScore', () => {
    it('projects PeerlensProfile to slim cache shape', () => {
      const client = new PeerlensQueryClient();

...
describe('notification_frame (task 4.40)', () => {
  ...
  describe('buildNotificationFrame', () => {
    ...
    it('accepts all 3 priorities', () => {
      for (const priority of ['fiduciary', 'solicited', 'engagement'] as const) {
        expect(buildNotificationFrame({ ...validInput, priority }).priority).toBe(priority);
      }
    });
    ...
    it.each([
      ...
      expect(() => buildNotificationFrame({ ...validInput, ts })).toThrow(/ts must be/);
    });
  ...
  describe('isNotificationFrame type guard', () => {
    ...
    it.each([
      ...
      [
        'wrong v',
        { type: 'notification', v: 2, priority: 'fiduciary', message: '', ts: 0, id: 'x' },
      ],
      [
        'bad priority',
        { type: 'notification', v: 1, priority: 'admin', message: '', ts: 0, id: 'x' },
      ],
  ...
  describe('parseNotificationFrame', () => {
    ...
    it('rejects wrong version (forward-compat guard)', () => {
      expect(
        parseNotificationFrame(JSON.stringify({ ...buildNotificationFrame(validInput), v: 2 })),
    ...
    it('rejects malformed field types', () => {
      ...
      expect(
        parseNotificationFrame(JSON.stringify({ ...buildNotificationFrame(validInput), id: '' })),
      ).toEqual({ ok: false, reason: 'invalid_field' });
    });
  });

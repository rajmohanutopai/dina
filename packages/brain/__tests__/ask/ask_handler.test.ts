...
describe('createAskHandler (task 5.17)', () => {
  ...
  describe('input validation', () => {
    it('empty question rejects', async () => {
      ...
      await expect(handler({ question: '', requesterDid: 'did:plc:u' })).rejects.toThrow(
        /question/,
      );
    });
    ...
    it('empty requesterDid rejects', async () => {
      ...
      await expect(handler({ question: 'q', requesterDid: '' })).rejects.toThrow(/requesterDid/);
    });
  });
  ...
  describe('events', () => {
    it('fast-path emits submitted(fastPath=true) + fast_path_complete', async () => {
      ...
      const submitted = events.find((e) => e.kind === 'submitted') as Extract<
        AskHandlerEvent,
        { kind: 'submitted' }
      >;
...
describe('createAskStatusHandler (task 5.18)', () => {
  ...
  it('renders failed status with decoded error', async () => {
    ...
    await registry.markFailed(rec.id, JSON.stringify({ kind: 'provider_error', message: '503' }));
    const handler = createAskStatusHandler({ registry });
  ...
  it('construction throws without registry', () => {
    expect(() => createAskStatusHandler({ registry: undefined as unknown as AskRegistry })).toThrow(
      /registry/,
    );
  });
});

...
export interface AskRecord {
  ...
  approvalId?: string;
  /**
   * Pattern A suspend payload: JSON-encoded `PausedAgenticState`
   * captured at the moment the agentic loop bailed on
   * `approval_required`. Present iff `status === 'pending_approval'`
   * AND the producer was the agentic loop (Pattern A); persona_guarded
   * Pattern B callers pass only `approvalId` and leave this absent.
   *
   * Stored as opaque JSON so the registry is agnostic of the loop's
   * schema. The resumer parses + version-checks on read. Cleared on
   * any terminal transition (complete / failed / expired) and on
   * `resumeAfterApproval` so the resume flow is the only consumer.
   */
  pausedStateJson?: string;
}
...
export class InMemoryAskAdapter implements AskPersistenceAdapter {
  ...
  async insert(record: AskRecord): Promise<void> {
    if (this.rows.has(record.id)) {
      throw new Error(`InMemoryAskAdapter.insert: duplicate id ${JSON.stringify(record.id)}`);
    }
  ...
  async update(record: AskRecord): Promise<void> {
    if (!this.rows.has(record.id)) {
      throw new Error(`InMemoryAskAdapter.update: unknown id ${JSON.stringify(record.id)}`);
    }
    this.rows.set(record.id, cloneAsk(record));
  }
...
export class AskRegistry {
  ...
  constructor(opts: AskRegistryOptions) {
    ...
    if (!Number.isFinite(ttl) || ttl <= 0) {
      throw new Error(`AskRegistry: defaultTtlMs must be > 0 (got ${ttl})`);
    }
  ...
  async markComplete(id: string, answerJson: string): Promise<AskRecord> {
    ...
    delete record.pausedStateJson; // terminal — paused state no longer relevant
   ...
   *
   * `pausedStateJson` is the optional Pattern A suspend payload — the
   * agentic loop's serialized `PausedAgenticState`. Pattern B callers
   * (persona-guarded executeFn) leave it absent; the resumer falls
   * back to re-running the original question. Stored opaquely; the
   * registry never parses it.
   */
  async markPendingApproval(
    ...
    if (pausedStateJson !== undefined) {
      record.pausedStateJson = pausedStateJson;
    } else {
      delete record.pausedStateJson;
    }
    await this.adapter.update(record);
   ...
   *
   * **Note**: `pausedStateJson` is NOT cleared here — the resumer
   * needs to read it to restart the agentic loop. The resumer clears
   * it implicitly via the next terminal transition (markComplete /
   * markFailed) or another markPendingApproval (which overwrites or
   * clears based on whether a new payload is supplied).
   */
  ...
  async sweepExpired(): Promise<number> {
    ...
    for (const row of rows) {
      if (
        ...
        row.status = 'expired';
        row.updatedAtMs = now;
        delete row.pausedStateJson; // terminal — drop the suspend payload
  ...
  async purge(id: string): Promise<boolean> {
    const record = await this.requireRecord(id);
    if (record.status !== 'complete' && record.status !== 'failed' && record.status !== 'expired') {
      return false;
    }
...
function cloneAsk(r: AskRecord): AskRecord {
  ...
  if (r.pausedStateJson !== undefined) clone.pausedStateJson = r.pausedStateJson;
  return clone;
}

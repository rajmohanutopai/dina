...
function validate(input: PersonaGateInput): string | null {
  ...
  if (typeof input.persona.name !== 'string' || input.persona.name === '')
  ...
  if (input.sessionGrant !== undefined) {
    ...
    if (!Number.isFinite(input.sessionGrant.expiresAtSec))
      return 'sessionGrant.expiresAtSec must be finite';
  }

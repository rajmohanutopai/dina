 ...
 *     and pre-fill the subject record)
 *   - AppView's subject enricher (parse `subject.identifier` into a
 *     typed canonical form for cross-attestation deduplication)
 *
 * Formats covered:
 *   - DOI            10.<registrant>/<suffix>
 *   - arxiv          new (YYMM.NNNNN) and old (archive/YYMMNNN)
 *   - ISBN-13        13 digits, modulo-10 weighted check
 *   - ISBN-10        10 chars, last optionally 'X', modulo-11 check
 *   - EAN-13         13 digits, modulo-10 weighted check (same algo
 *                    as ISBN-13 — ISBN-13 is a subset starting 978/979)
 *   - UPC-A          12 digits, modulo-10 weighted check
 *   - ASIN           10 alphanumeric, no checksum (Amazon-internal)
 ...
 *
 * Zero runtime deps — all string + arithmetic.
 */
...
function stripSeparators(s: string): string {
  ...
  // identifiers: spaces, hyphens, em/en dashes. We do NOT strip
  // case here — UPC/ASIN/etc are case-sensitive in different ways.
 ...
 * where weights are 1, 3, 1, 3, ..., 1 across positions 0..12.
 */
 ...
 * character may be 'X' (= 10). Hyphens and spaces are stripped by
 * the caller.
 */
...
export function parseDoi(input: string): ParsedIdentifier | null {
  ...
  let body = raw;
  // Strip common URL / scheme prefixes (case-insensitive).
  body = body.replace(/^doi:\s*/i, '');
  body = body.replace(/^https?:\/\/(?:dx\.)?doi\.org\//i, '');
  // DOIs are case-insensitive but conventionally lower-cased.
  // Pattern: `10.<registrant>/<suffix>` where registrant is digits
  // (4–9 typical) and suffix is non-empty, no whitespace.
...
/**
 * Parse an arxiv identifier. Recognises both modern
 * (`YYMM.NNNNN`, optional `vN` suffix) and pre-2007 (`archive/YYMMNNN`)
 * forms. The canonical value drops any `arXiv:` prefix.
 */
export function parseArxiv(input: string): ParsedIdentifier | null {
  const raw = input.trim();
  const body = raw.replace(/^arxiv:\s*/i, '');
  // Modern: 4-digit YYMM + dot + 4 or 5 digits + optional vN
  if (/^\d{4}\.\d{4,5}(?:v\d+)?$/.test(body)) {
    return { type: 'arxiv', value: body.toLowerCase(), raw };
  }
  // Pre-2007: archive/YYMMNNN with optional .subject prefix on the
  // archive name (e.g. `cond-mat/9612001`).
...
/**
 * Parse an ISBN-13. Hyphens/spaces stripped; canonical value is the
 * 13-digit form. Rejects ISBN-13s that are also valid EAN-13s but
 * don't start with the 978/979 ISBN registrant prefixes.
 */
...
/**
 * Parse an ISBN-10. Hyphens/spaces stripped; canonical value is the
 * 10-character form with the optional final `X` upper-cased.
 */
...
/**
 * Parse an EAN-13. Same checksum as ISBN-13; the difference is that
 * EAN-13 covers the broader product-barcode space (everything that
 * isn't 978/979 ISBN). Canonical value is the 13-digit form.
 */
export function parseEan13(input: string): ParsedIdentifier | null {
  ...
  if (body.length !== 13 || !isDigitsOnly(body)) return null;
  if (!ean13Checksum(body)) return null;
  // Don't shadow ISBN-13: callers wanting ISBN-13 should use parseIsbn13.
  // parseIdentifier orders ISBN-13 first; here we accept any valid 13-digit
  // EAN, which is structurally a superset.
 ...
 *
 * No checksum is publicly defined; we validate structure only.
 * The mixed-alphanum requirement filters two whole classes of
 * false positive:
 *   - pure-digit input (would clash with ISBN-10)
 *   - pure-letter 10-character words (real ASINs always contain
 *     digits — historically every ASIN issued by Amazon includes
 *     digits, since they're internally sequential)
 *
 * Pure-digit ISBN-10s should go through `parseIsbn10`.
 */
...
/**
 * Parse a Google Place ID. Place IDs are opaque, server-issued
 * tokens — there's no public checksum. Modern IDs start with `ChIJ`
 * or `Eo` and are at least 20 chars of URL-safe base64. We do
 * structural validation only; the AppView enricher resolves the
 * actual place via the Places API at indexing time.
 */
 ...
 *   7. ASIN      — 10 alphanumeric (rejects pure-digit to avoid ISBN-10 clash)

...
describe('@dina/protocol/peerlens types (TN-PROTO-001)', () => {
  ...
  it('PeerlensNsid union covers every value in PEERLENS_NSIDS', () => {
    // Compile-time exhaustiveness: assigning every NSID to PeerlensNsid
    // would fail if a value were missing from the union.
    const ids: PeerlensNsid[] = Object.values(PEERLENS_NSIDS);
  ...
  it('SubjectClaim / PeerlensPolicy / NotificationPrefs compile', () => {
    ...
    const tp: PeerlensPolicy = { maxGraphDepth: 3, requireVouch: false, createdAt: ISO };

...
The CLI used for OpenClaw is paired with `--role agent` (`cli/src/dina_cli/main.py:598`). This means ALL writes from this CLI identity land as `(cli, agent)` → `unknown / service / medium / caveated` in the PeerLens scorer (`brain/src/service/trust_scorer.py:109`). No identity separation problem — the CLI IS the agent.

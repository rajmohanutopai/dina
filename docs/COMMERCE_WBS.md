...
| 3.2 | Provider-ingress bridge | §11.2a | **PART** | 3.1 | Wired: `ServiceHandler` recognises the plugin plane and routes through an injected `ProviderIngressSubmitter`, defaulted in `buildHomeNodeServiceRuntime` so both boots get it. Auto and post-approval paths both dispatch; typed refusals reach the requester. **Open:** the completion side (runner result → pinned result schema → D2D response as Business DID) is not driven end to end by a test |

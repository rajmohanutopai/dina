...
| 2.4 | Epoch service — CAS publication, restore increment, fail-closed signing | §16.2 | **PART** | 2.1 | Service now has a real repo: `PDSPublisher.getRecord` + `putRecord(..., {swapRecord})` over `com.dinakernel.commerce.epoch`, wired at server boot and installed only on success. A node with no PDS leaves commerce disabled. **Open:** no periodic live-epoch revalidation, so a forgotten pre-restore node signs at its cached epoch indefinitely; mobile boot has no epoch wiring |
...
| 3.4 | Extension-operation broker | §3.4 | **PART** | 3.1 | State machine built: durable `plugin_extension_proposals` ledger; propose (deny-before-validation, params checked before the row exists, idempotent by (install, key)); permit / refuse-recorded / cancel-before-decision; begin-execution as its own crash boundary; settle verifying against the schema pinned AT PROPOSAL TIME; `outcome_unknown` terminal and distinct from failed; `listExecuting` for the sweeper. 7 mutations caught. Composed at BOTH boots via `createPluginHostRuntime`, with a static guard that nothing else constructs the parts. **Open:** no owner-facing permit surface; no sweeper wired over `listExecuting()`; no runner path calls `propose` yet |
| 3.5 | Typed host operations (bounded AppView search, D2D send, publication candidate, connector broker) | §3.4 FR-P9 | **PART** | 3.4 | `HostOperationDispatcher` drives permitted proposals: claim `executing` before the effect, run, settle against the pinned result schema. Executor context carries params + install identity and NOTHING credential-shaped, so a runner cannot smuggle authority through a payload. A thrown executor settles `outcome_unknown` (a socket can die after the bytes left); characterised failures return `failed`. Bounded AppView search truncates in Core and the request has no limit field to widen. 8 mutations caught. Composed at both boots. **Open:** only the search operation exists — D2D send, publication candidate and connector broker are not written; no boot registers an executor for it |
| 3.6 | Composition root — production wiring for admission / lifecycle / epoch on both boots | §23 | **PART** | 0.2 0.3 0.5 3.1 | Root builds admission + lifecycle, ties acceptance to genesis (§12.8), shares the Tier-0 runner, and answers a typed `commerceAvailability()`. Both boots get the engines with no boot-side change. **Open:** nothing installs `CommerceEpochService`, so a real boot reports `no_epoch` — commerce is cleanly disabled, not live. Needs the CAS-capable PDS publisher (2.4) |
...
| 4.2 | **Validated commerce archive restore** — operational tables restored as EVIDENCE, counters never adopted *(owner decision 2026-08-07)* | §16.2 | **PART** | 0.2 0.3 2.4 | The fence is armed and discharged: the import writes a durable marker in its OWN transaction; boot reads it and takes `establishAfterRestore` (epoch increment + capacity void + fence receipt) instead of adopting the live epoch; the marker clears only after publication succeeds and is left set on failure so the next boot retries. Unreadable kv reads as "fence owed". Not exported. 5 mutations caught. **Open:** archive preflight (row/digest/required-table/cross-table integrity proof) is not implemented — restore currently trusts the archive's structure and relies on the fence for the counter half |
| 4.3 | Per-order restore reconciliation ceremony | §16.2 | **PART** | 4.2 0.3 | `reconcileRestoredOrder` clears `reconciliation_required` against the buyer-presented order proposal, digest-bound to the supplier's own acknowledgement, and stores it so the order can describe itself again. Layered aggregate check + repository CAS. **Open:** no owner-facing surface (route/UI) and no bulk sweep over all non-terminal orders — the ceremony is per-order and caller-driven. 4 mutations caught |
...
| 4.5 | Pause / uninstall / update semantics | §16.3 §16.4 §16.5 | **PART** | 3.7 | Uninstall REFUSES while commerce obligations are open (undecided orders + non-terminal chains), with a typed error carrying the count; `delivered` inside its dispute window counts as open. 4 mutations caught. **Open:** the count is node-wide because an order reference does not record which install served it (conservative, over-refuses on multi-plugin nodes); pause and update semantics not yet addressed |

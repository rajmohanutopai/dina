...
| 0.2b | Extract `AdmissionService` as a separate transactional coordinator | §9.9 | **TODO** | 0.2 | Engines stop owning the transaction; raw `CommerceOrderRefRepository` unreachable from production |
...
| 0.5 | Ledger boundary made real: stop exporting raw repositories; replace the mutable global with a composition factory | §16.2 | **TODO** | 0.2 0.3 | Package root exports aggregates only; `get/setCommerceQuoteLedgerRepository` removed; static forbidden-import check in CI |
...
| ID | Deliverable | Spec | Status | Depends | Definition of Done |
|----|-------------|------|--------|---------|--------------------|
| 3.1 | Provider-kind install support | §11.2a §23 | **TODO** | — | `NODE_SUPPORTED_FEATURES` accepts provider manifests; a Supplier manifest installs through the public API |

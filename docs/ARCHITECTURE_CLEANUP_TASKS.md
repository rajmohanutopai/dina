...
| CONFIG-001 | done | config | Centralize test/release endpoint resolution. | `@dina/home-node` now owns hosted endpoint mode selection, mobile/server env keys, URL validation, PDS host derivation, and MsgBox/PDS/AppView/PLC defaults. Mobile MsgBox, onboarding handle/PLC defaults, HandlePicker/OwnerName, PeerLens AppView, node-runtime AppView, and `@dina/net-expo` use it. | `@dina/home-node`, mobile onboarding tests, boot capability tests, mobile typecheck, and `@dina/net-expo` typecheck pass. |

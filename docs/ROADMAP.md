...
| 1.10b | Admin backend (`dina-admin`) | Infra | Dedicated FastAPI backend for browser/admin traffic. Browser authenticates to `dina-admin` with a session cookie. `dina-admin` authenticates to core with its own Ed25519 service key. Externally reachable at `/admin` via reverse proxy, but core is transport only — no browser-session translation, no `CLIENT_TOKEN` injection. Dashboard (connector status, vault health, brain responsiveness), nudge history, contacts/sharing rules, settings/personas. **Build early — a UI to inspect vault, connectors, and nudges accelerates development of everything that follows.** | 1.10, 1.10a | dina-admin container | NOT STARTED |
...
| 1.22a | Migration CLI (`dina export/import`) | L1 Storage | `dina export` bundles all node state (persona SQLite files, encrypted master key, salt, media, config) into a single AES-256-GCM encrypted `.dina` archive. `dina import` restores on new machine after passphrase verification. Excludes service keys, OAuth tokens, device tokens (regenerated on new machine). Enables zero-lock-in migration between hosting levels. | 1.7, 1.8, 1.21 | dina-core | NOT STARTED |
...
| 1.29b | Watchdog & self-notifications | Infra | **Internal Go ticker (not Prometheus).** Lightweight background goroutine in dina-core checks system health every hour. Tracks: OpenClaw health (last successful MCP sync), disk usage, brain responsiveness, vault size per persona file. When thresholds breach (OpenClaw unreachable > 48h, disk > 90%, brain unresponsive), injects Tier 2 system message into user's notification stream: "Gmail hasn't synced in 48 hours. Is OpenClaw running?" No external monitoring stack needed — zero extra RAM, zero user setup, works on Raspberry Pi. **Optional:** `/metrics` endpoint (Prometheus format, protected by admin session or operator-local auth) for power users with existing homelab dashboards. | 1.7, 1.16 | dina-core | NOT STARTED |
...
| **1a** | Sidecar running (Cloud LLM) | Core, brain, admin, and PDS running; brain reasons via Gemini Flash Lite + Deepgram STT, core stores, admin UI is served by `dina-admin`, PDS hosts trust data, YouTube analysis via ADK |

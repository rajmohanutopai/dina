...
The roadmap below changes the target admin shape:

- extract browser/admin traffic into a dedicated `dina-admin` backend
- remove `CLIENT_TOKEN` from the canonical browser/core auth path
- keep Ed25519 as the only service-to-core authentication model
...
|---|------|-------|-----------|-----------|-----------|--------|
| 1.1 | Cloud API setup | Infra | Configure Gemini 2.5 Flash Lite API key (text LLM), Deepgram Nova-3 API key (voice STT), `gemini-embedding-001` (embeddings). Brain's LLM router calls cloud APIs in Cloud LLM profile (default). | — | Cloud APIs | DONE |
| 1.2 | dina-brain skeleton | L6 Intelligence | Python + Google ADK, basic agent loop, `/v1/process` and `/v1/reason` endpoints on port 8200. LLM router calls Gemini Flash Lite for text, Deepgram WebSocket for voice STT. | 1.1 | dina-brain | DONE |
...
| 1.5 | Silence filter (basic) | L6 Intelligence | Three-priority classification (Fiduciary / Solicited / Engagement) using Gemini Flash Lite | 1.2, 1.1 | dina-brain | DONE |
| 1.6 | LLM routing in brain | L6 Intelligence | Simple → Flash Lite, Complex → Flash/Pro/Claude, Voice → Deepgram STT. Sensitive personas → llama if available (best privacy), otherwise Entity Vault scrubbing (Tier 1+2 mandatory) then cloud LLM. Cloud sees topics, not identities. | 1.1, 1.2 | dina-brain | DONE |
...
| 1.8 | DID key management in core | L0 Identity | BIP-39 seed → Master Seed, key wrapping (passphrase → Argon2id KEK → AES-256-GCM wraps seed → `wrapped_seed.bin`). SLIP-0010 persona key derivation (Ed25519 signing keys). Per-persona vault DEKs via HKDF from master seed with persona-specific info strings (`"dina:vault:identity:v1"`, `"dina:vault:personal:v1"`, etc.). `/v1/did/sign` and `/v1/did/verify` endpoints. Selective persona unlock: `POST /v1/persona/unlock {persona, ttl}` → core derives DEK, opens database file, auto-closes after TTL. Go: `crypto/ed25519`, `x/crypto/argon2`, `stellar/go/exp/crypto/derivation`. | 1.7 | dina-core | DONE |
| 1.9 | PII scrubber (three-tier) | L6 Intelligence | **Tier 1:** Regex-based PII detection in Go core (credit cards, phone numbers, Aadhaar, emails). `/v1/pii/scrub` endpoint. Returns replacement map (`[PERSON_1]` → original) for de-sanitization. **Tier 2:** spaCy NER in Python brain (`en_core_web_sm`, ~15MB) catches contextual PII (names, orgs, locations). Always available, no llama required. **Tier 3 (optional):** LLM NER via llama for edge cases. | 1.7, 1.2 | dina-core + dina-brain | DONE |
| 1.10 | docker-compose wiring | Infra | Canonical orchestration target: 4 containers (core, brain, admin, pds — always on). Optional llama via `--profile local-llm` (5th container). Internal network. Brain and admin both call core API; brain also calls cloud APIs (Gemini, Deepgram). Single `docker compose up -d` for Cloud LLM profile, `docker compose --profile local-llm up -d` for Local LLM. Healthchecks (`/healthz`, `/readyz`) on all containers. Dependency chain: brain/admin wait for core healthy. `restart: always`. Structured JSON logging (Go `slog`, Python `structlog`) to stdout. Vault passphrase via Docker Secrets (tmpfs-mounted, never in env vars or CLI). | 1.2, 1.7, 1.1 | All | DONE |
...
| 1.10a | Gatekeeper: service→core auth | Infra | **Core stays the kernel.** No JWTs. Internal services (`dina-brain`, `dina-admin`, Telegram/OpenClaw-facing connectors) authenticate to core with service-specific Ed25519 keys. Paired client devices authenticate with Ed25519 device signatures. Browser auth terminates at `dina-admin` via session cookie and never uses `CLIENT_TOKEN`. Static allowlists in core enforce per-service capabilities: brain gets reasoning/storage capabilities, admin gets explicit admin capabilities, connectors get narrow ingestion capabilities. Brain never calls `/v1/did/sign` directly — it triggers high-level ops and core handles crypto internally. Persona access tiers also enforced: open (serve + log), restricted (serve + log + notify), locked (reject until user unlocks with TTL). | 1.7, 1.2 | dina-core + service callers | DONE |
| 1.10b | Admin backend (`dina-admin`) | Infra | Dedicated FastAPI backend for browser/admin traffic. Browser authenticates to `dina-admin` with a session cookie. `dina-admin` authenticates to core with its own Ed25519 service key. Externally reachable at `/admin` via reverse proxy, but core is transport only — no browser-session translation, no `CLIENT_TOKEN` injection. Dashboard (connector status, vault health, brain responsiveness), nudge history, contacts/sharing rules, settings/personas. **Build early — a UI to inspect vault, connectors, and nudges accelerates development of everything that follows.** | 1.10, 1.10a | dina-admin container | DONE |
...
| 1.19 | Basic Dina-to-Dina messaging | L4 Dina-to-Dina | Send/receive encrypted messages between two Home Nodes. Sender FS via ephemeral keys. DIDComm-compatible plaintext format. | 1.17, 1.18 | dina-core | IN PROGRESS |
...
| 1.20 | **The Sancho Moment (E2E)** | All | Sancho's Dina sends "leaving home" → your core receives → brain checks vault for Sancho context → brain assembles nudge ("his mother was ill, put the kettle on") → core pushes to your phone. | 1.19, 1.15, 1.16 | All | IN PROGRESS |
...
| 1.28 | Local bot trust tracking | L3 Trust | Track bot accuracy, response time, uptime locally. Route to better bots when quality drops. | 1.27 | dina-brain | NOT STARTED |
| 1.29 | Client authentication | Infra | **Ed25519 device keypair pairing.** **First device:** `docker compose up` prints local IP + 6-digit pairing code to terminal (expires in 5 min). Device (phone, CLI, laptop) generates Ed25519 keypair, finds Home Node via mDNS (`github.com/hashicorp/mdns`) or manual IP entry. User enters pairing code → device sends public key → core registers in `identity.sqlite` `device_tokens` table → device signs every request with Ed25519. **Browser admin is separate:** browser authenticates to `dina-admin` with a session cookie; `dina-admin` authenticates to core with Ed25519. **Managed hosting:** signup flow provides pairing code. **Subsequent devices:** same flow — new pairing code via admin UI or terminal. Each device gets its own Ed25519 keypair, independently revocable. **QR code deferred** — cosmetic, not needed for Phase 1. All communication over TLS + Ed25519 signed auth frame on WebSocket. | 1.8, 1.17 | dina-core | DONE |
...
| 1.29b | Watchdog & self-notifications | Infra | **Internal Go ticker (not Prometheus).** Lightweight background goroutine in dina-core checks system health every hour. Tracks: OpenClaw health (last successful MCP sync), disk usage, brain responsiveness, vault size per persona file. When thresholds breach (OpenClaw unreachable > 48h, disk > 90%, brain unresponsive), injects Tier 2 system message into user's notification stream: "Gmail hasn't synced in 48 hours. Is OpenClaw running?" No external monitoring stack needed — zero extra RAM, zero user setup, works on Raspberry Pi. **Optional:** `/metrics` endpoint (Prometheus format, protected by admin session or operator-local auth) for power users with existing homelab dashboards. | 1.7, 1.16 | dina-core | DONE |
...
| 1.35 | Telegram connector | L2 Ingestion | Telegram Bot API — user creates bot via @BotFather, configures token in Dina. Home Node receives messages via webhook/long polling. Full message content, media, group context. Server-side, cross-platform. | 1.30 | dina-core + dina-brain | DONE |
| 1.36 | Agent delegation (OpenClaw) | L7 Action | Delegate tasks to OpenClaw and other child agents via MCP. License renewal, form filling, task automation — all with `draft_only` constraint. No plugins — agents are external processes. | 1.25 | dina-brain | IN PROGRESS |
...
| 3.6 | Direct value exchange | L3 Trust | Creators earn when their reviews drive purchases. Truth pays better than clicks. Micropayments via UPI/crypto. | 3.5, 3.4 | NOT STARTED |
...
| **1a** | Sidecar running (Cloud LLM) | **DONE** — Core, brain, admin UI running; LLM routing, PII scrubbing, silence classification, docker-compose, gatekeeper auth, task queue, staging pipeline |
...
| (architectural decision) Dedicated admin backend | 1a | Issue #5 — browser/admin traffic is extracted from brain into `dina-admin`. Brain stays focused on reasoning/orchestration; admin owns browser sessions and HTML/API responses. |
| (architectural decision) Browser auth gateway | 1a | Issue #6 — `dina-admin` serves the login page, validates passphrase via a privileged core call over Ed25519, sets HttpOnly/Secure/SameSite=Strict session cookie, and owns browser session state. Core may still reverse-proxy `/admin`, but it does not translate browser sessions into bearer tokens. |
...
| Container model was inconsistent | Fixed: roadmap now distinguishes current implementation (3+1) from canonical target (4+1 with dedicated `dina-admin`). llama remains optional in both cases. |

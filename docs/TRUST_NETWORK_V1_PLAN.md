...
| 17 | **the PeerLens tab default landing = friends' recent reviews (1-hop, 14-day window)**, served by `com.dinakernel.peerlens.networkFeed` xRPC | Teaches social-review framing on first open; doesn't require typing a query |
| 18 | **Compose flow calls `com.dinakernel.peerlens.resolve` before publish** | Server returns canonical `subject_id` (or null + conflicts list for disambiguation); UI shows the canonical subject before the user commits |
...
| Namespace key registration? | New namespace = mobile derives key + appends `assertionMethod` to DID document via PLC operation signed by recovery key + publishes `com.dinakernel.peerlens.namespaceProfile` | See §3.5 for sequence |
...
| Empty `q` parameter behaviour? | Returns the network feed via `com.dinakernel.peerlens.networkFeed` (1-hop reviewers, 14d window, sorted by reviewer trust × recency) | Same data the PeerLens tab landing renders |
...
| Feature flag for V1 rollout? | `appview_config.trust_v1_enabled` boolean. When `false`, all `com.dinakernel.*` xRPC endpoints return HTTP 503 and the firehose ingester skips trust-network lexicon records | Lets us dark-launch the schema without exposing UI |
...
**Lexicon namespace decision (TN-DEC-001):** V1 keeps the existing `com.dinakernel.peerlens.*` namespace already in production AppView. The plan was originally drafted with `com.dinakernel.subject.*` names; those have been renamed throughout the doc to match the existing namespace. Lexicons §3.1–§3.3 (attestation, endorsement, flag) **already exist** in `appview/src/ingester/handlers/` — V1 work modifies them in place rather than creating new lexicons. §3.4 `namespaceProfile` is genuinely net-new; it's a V1 addition.
...
### 3.1 `com.dinakernel.peerlens.attestation`
...
### 3.2 `com.dinakernel.peerlens.endorsement` (cosignature)

> **Note:** The plan originally named this primitive "binding". V1 uses the existing AppView lexicon `com.dinakernel.peerlens.endorsement` — same concept (a cosignature on another reviewer's attestation), existing field names take precedence.

```json
{
  "id": "com.dinakernel.peerlens.endorsement",
  "record": {
...
### 3.3 `com.dinakernel.peerlens.flag` (V1 record exists, V1 UI defers)

```json
{
  "id": "com.dinakernel.peerlens.flag",
  "record": {
...
### 3.4 `com.dinakernel.peerlens.namespaceProfile` (per pseudonymous identity)

```json
{
  "id": "com.dinakernel.peerlens.namespaceProfile",
  "record": {
...
6. Mobile publishes `com.dinakernel.peerlens.namespaceProfile{name, verificationMethodId, createdAt}` record signed by the new namespace key
...
2. Mobile publishes a delete-and-republish of `com.dinakernel.peerlens.namespaceProfile` with `disabled: true` and the same `verificationMethodId`
...
// type=did
{
  ...
  is_service?: boolean;         // has com.dinakernel.service.profile record
...
**Type=did:**
- AppView resolves the DID once at ingest; if a `com.dinakernel.service.profile` record exists in the resolved repo, sets `metadata.is_service = true` and copies the published capability label.
...
All read endpoints under `com.dinakernel.*`, served by the AppView Web daemon (port 3000). Writes happen at the user's PDS via `com.atproto.repo.createRecord` (see §3.5.1). AppView's role is read-side indexing + querying only.

| Endpoint | Kind | Auth | Rate limit |
|---|---|---|---|
| `com.dinakernel.peerlens.search` | Read (xRPC query) | Public | 60 / IP / min |
| `com.dinakernel.peerlens.subjectGet` | Read (xRPC query) | Public | 120 / IP / min |
| `com.dinakernel.peerlens.resolve` | Read (xRPC query) | Public | 60 / IP / min |
| `com.dinakernel.peerlens.networkFeed` | Read (xRPC query) | Public, viewerDid mandatory | 60 / IP / min |
| `com.dinakernel.peerlens.attestationStatus` | Read (xRPC query) | Public | 600 / IP / min (high — outbox polls every 5 s) |
| `com.dinakernel.peerlens.cosigList` | Read (xRPC query) | Public, recipientDid filter | 60 / IP / min |
...
**Ingester gates** (run for every `com.dinakernel.*` record received from Jetstream):
...
### 6.1 `com.dinakernel.peerlens.search` (query)
...
### 6.2 `com.dinakernel.peerlens.subjectGet` (query)
...
### 6.3 `com.dinakernel.peerlens.resolve` (query)
...
### 6.4 `com.dinakernel.peerlens.networkFeed` (query)
...
### 6.5 `com.dinakernel.peerlens.attestationStatus` (query)
...
### 6.6 `com.dinakernel.peerlens.cosigList` (query)
...
The landing screen is `app/trust/index.tsx`; it renders the `com.dinakernel.peerlens.networkFeed` response (1-hop reviewers' recent attestations, 14-day window) above the search input. Tapping a feed row deep-links into `[subjectId]`. When the viewer has no contacts yet, the feed swaps to a "Add contacts to see their reviews" prompt with a button to People tab.
...
Reviewer's namespace name is `name` from `com.dinakernel.peerlens.namespaceProfile` if the namespace is visible (i.e., DID document includes the verification method). For namespaces the viewer can't yet see, falls back to "Pseudonymous reviewer · #namespace_K".
...
Recovery: master seed regenerates all namespace keys deterministically. Namespace metadata (display name, optional public handle) lives in `com.dinakernel.peerlens.namespaceProfile` records on the user's PDS — recovered by re-resolving the user's repo.
...
| Flag UI | Existing `com.dinakernel.peerlens.flag` records | UI surfaces; scorer absorbs flagged-count signal |
...
- All `com.dinakernel.*` xRPC endpoints return HTTP 503 with body `{error: 'trust_v1_disabled'}`
- Firehose ingester skips trust-network lexicon records (`com.dinakernel.peerlens.attestation`, `endorsement`, `flag`, `namespaceProfile`)
...
- 🔧 Extend existing `com.dinakernel.peerlens.attestation` lexicon — add namespace field semantics (no schema break)
- 🔧 Extend existing `com.dinakernel.peerlens.endorsement` lexicon — confirm cosignature semantics match plan §3.2
- 🔧 Extend existing `com.dinakernel.peerlens.flag` lexicon — confirm matches plan §3.3, add ingester-level rate-limit
- ✨ New: `com.dinakernel.peerlens.namespaceProfile` lexicon (one of the few genuinely-new artifacts in V1)
...
- 🔧 `pds_publish.ts` (signs + publishes attestations) — already implemented; **needs lexicon NSID update from `community.dina.trust.attestation` → `com.dinakernel.peerlens.attestation`**, and SubjectRef extension (currently DID-only)
...
- **Endorsement** — a cosignature on someone else's attestation (V1 endorse-only); existing AppView lexicon `com.dinakernel.peerlens.endorsement`. Plan-internal name "binding" is retired

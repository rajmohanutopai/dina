...
- **Service keys** are Ed25519 keypairs generated at first startup by `install.sh`. Private keys are isolated by separate Docker bind mounts: `secrets/service_keys/core/` is mounted only to Core (as `/run/secrets/service_keys/private/`), `secrets/service_keys/brain/` is mounted only to Brain (as `/run/secrets/service_keys/private/`), and `secrets/service_keys/public/` is mounted to both containers (as `/run/secrets/service_keys/public/`). Each container sees only its own private key and both services' public keys — Core's private key never exists in Brain's filesystem and vice versa. This isolation is enforced at the container level, not by file permissions. Brain signs every request to core with its private key. No shared secret — authentication is cryptographic. CLIENT_TOKEN is used only for admin web UI login, stored as SHA-256 hash in `identity.sqlite` `device_tokens` table. All client devices use Ed25519 keypairs (no shared secret).

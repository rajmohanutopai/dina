...
services:
  ...
  core:
    ...
    volumes:
      ...
      - ./secrets/service_keys/core:/run/secrets/service_keys/private    # Core's private key only
      - ./secrets/service_keys/public:/run/secrets/service_keys/public   # Both services' public keys
  ...
  brain:
    ...
    # SERVICE KEYS: Ed25519 keypair for authenticating requests to core
    # Private key isolated — Brain cannot see Core's private key
    volumes:
      - ./secrets/service_keys/brain:/run/secrets/service_keys/private   # Brain's private key only
      - ./secrets/service_keys/public:/run/secrets/service_keys/public   # Both services' public keys
...
# 2. Generate Ed25519 service keys (each container gets its own keypair)
# Private keys isolated by bind mount — core/ and brain/ are mounted separately
...
- **Service keys** are Ed25519 keypairs derived deterministically from the master seed at install time via SLIP-0010 (`install.sh` calls `provision_derived_service_keys.py`). Core is at `m/9999'/3'/0'`, Brain at `m/9999'/3'/1'`. At runtime, services load existing keys and fail if they are missing — no key generation occurs. Private keys are isolated by separate Docker bind mounts: `secrets/service_keys/core/` is mounted only to Core (as `/run/secrets/service_keys/private/`), `secrets/service_keys/brain/` is mounted only to Brain (as `/run/secrets/service_keys/private/`), and `secrets/service_keys/public/` is mounted to both containers (as `/run/secrets/service_keys/public/`). Each container sees only its own private key and both services' public keys — Core's private key never exists in Brain's filesystem and vice versa. This isolation is enforced at the container level, not by file permissions. Brain signs every request to core with its private key. No shared secret — authentication is cryptographic. CLIENT_TOKEN is used only for admin web UI login, stored as SHA-256 hash in `identity.sqlite` `device_tokens` table. All client devices use Ed25519 keypairs (no shared secret).
...
Four things total: domain, API key, OpenClaw URL, vault mode, plus secrets generated by `install.sh` (passphrase, service keys). No OAuth credentials needed — OpenClaw manages external API auth. Developer fills in `.env`, runs `./install.sh`, runs `docker compose up`, has a working Dina.
...
TAILSCALE AUTH (SERVER-INITIATED):
  ...
  COEXISTENCE:
    All four auth methods (ED25519_SERVICE_KEY, CLIENT_TOKEN, ED25519_SIGNED, TAILSCALE)

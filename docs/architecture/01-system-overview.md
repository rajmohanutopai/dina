...
**Why this matters for architecture:** No plugin store to maintain, no plugin review process, no sandboxing, no scoped tokens, no plugin API versioning. The three-tier auth model (Ed25519 service keys + `CLIENT_TOKEN` + Ed25519 device signatures) is the permanent design, not a stepping stone. CLI uses Ed25519 exclusively; CLIENT_TOKEN serves the admin web UI only. NaCl (for peers) and MCP (for agents) are the only extension points.

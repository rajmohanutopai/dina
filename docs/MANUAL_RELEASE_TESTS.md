...
- Paired `dina-agent` for task and agent-safety tests. Pairing is one-paste: app **Settings →
  Agents → Generate Setup Code** → on the agent host `dina init` (or `dina configure`) and paste
  the `dina1:…` string. There is no numeric-code fallback. Runners: `openclaw` (Gateway),
  `openclaw-cli`, `claude-code`, `codex`, `gemini`, or stub runners — select via
  `dina agent-daemon --runner <name>`.
...
| MRS-06 | 4. Task via agent | Pair `dina-agent` (Agents → **Generate Setup Code** → paste into `dina init`/`dina configure`; no numeric fallback exists). In Chat, give Dina a task. | Agent claims the task **through MsgBox** (never a direct connection), marks running, completes; result appears **once** in Chat/Activity. Brain itself never calls the tool/MCP directly. | MT-35,36,89 | |
...
| MRS-11 | 9. Services — known_only | Provider and A are mutual contacts. Provider issues a grant+offer (`POST /v1/service/offer` → D2D `service.offer` to A). In A's chat: "Ask <contact name> <the service question>". Then attempt the same listing from a non-grantee DID. | The offer rides A's contact through `find_preferred_provider` (grant_id + service_uri + schema hash are self-contained); A's query is **accepted** and the result card resolves. The non-grantee's query is **silently dropped** (never reaches the service runtime; requester sees expiry — the listing "doesn't exist" for them). The known_only listing appears in **neither** AppView search **nor** `getByUri` (actively unpublished from the PDS). Verified live 2026-06-10. | MT-63,79 | |
...
| MRS-GD-01 | Guided demo (data scope) | On a clean install tap **"See Dina in action"**. Complete the Emma memory, the chair query, and the service availability step; trigger the demo agent's Health request; then **Finish demo**. | Banner "Guided demo · sample data only" shows while active. Emma appears under **People/Relations** (not Contacts; no DID). Her **birthday reminder** appears and mentions dinosaurs (date = next upcoming Nov 7, year not hardcoded). The chair recommendation uses lower-back pain + $500 budget (recommends ErgoFlex, rejects BudgetLite/SpinePro); a service/PeerLens result card appears; the external service receives only `{product, location}` — never health/budget/person facts. The demo **agent** Health request shows a **real** approval card (owner Ask did NOT). After Finish, normal **Chat, People, Reminders are empty** — no demo data survives; export never contains demo rows. | MT-11,15,52,38 | data_scope leak + cleanup suites (core/scope, storage-node `*_scope_isolation`) |
...
| MT-35 | P0 | Agents | Pair dina-agent (setup code) | Generate Setup Code in the app; paste into `dina configure`/`dina init` pairs over MsgBox. Agent appears as active, can claim tasks, and all traffic goes through MsgBox. Expired/used setup code fails loudly and re-prompts. | |
...
| MT-78 | P1 | Services | Unlisted invoke by link | Paste an unlisted listing's `at://` URI into chat with the service question. The query resolves via getByUri (NOT search), executes, and the result card is attributed to the UNLISTED listing's own name. A schema difference vs a public sibling listing must not cause `schema_version_mismatch`. | |
| MT-79 | P1 | Services | Known-only invocation | Mutual contacts + provider offer (grant rides the contact). Invoke via chat "Ask <contact> …" succeeds; non-grantee query is silently dropped (expires); listing absent from search AND getByUri. A service contact added for the grant does NOT appear under People → Relations (Contacts only). | |
...
| MT-89 | P0 | Agents | One-paste setup code | `dina init` with a pasted `dina1:…` string pairs AND installs the skill in one run; `--setup-code` works headless; explicit flags override string fields; a corrupted string fails loudly (no silent manual fallback); the code expires ~5 min and is single-use. | |
| MT-90 | P1 | Agents | Skill install transparency | `dina skill install` detects platforms (~/.claude, ~/.openclaw, ~/.codex, ~/.gemini), prints every path BEFORE writing, honors --dry-run and per-platform confirmation; AGENTS.md/GEMINI.md edits stay inside the managed Dina block (re-install is idempotent, user content preserved byte-for-byte). | |
| MT-91 | P1 | Agents | Headless CLI runner | `dina agent-daemon --runner claude-code` (or codex/gemini/openclaw-cli) claims a task and the platform CLI executes it; result appears once in Chat/Activity; task description content cannot inject argv (single prompt argument); timeout surfaces the approval-persists hint. | |
| MT-92 | P1 | AI Provider | Key health pill | With a depleted/invalid key, Settings + Manage AI providers show the problem pill on the ACTIVE key (card icon = credits, key icon = rejected); chat failure lights the pill immediately; after fixing billing the next Settings visit clears it (≤15s error TTL); healthy keys show no pill. | |
| MT-93 | P0 | Durability | D2D / grant / quarantine survive restart | After a cold restart: a queued **D2D outbox** message still sends (not lost); a known_only **grant** issued before restart still authorizes the grantee's query; a **quarantine** accept/block decision persists (accepted sender no longer quarantines, blocked sender stays blocked). Schema v6/v7/v11 durability — these were in-memory-only before. | |
| MT-94 | P0 | Services | Visibility-save guard (taxonomy) | Saving a listing whose capability is **sensitive/subject-scoped** as **Public** is blocked with a clear alert offering **Make Unlisted / Make Private**; choosing one saves with that visibility. A non-sensitive public capability saves without friction. (Live-verified 2026-06-10.) | |
| MT-95 | P1 | Services | Listing lifecycle | A listing can be **created, edited, paused (drops from AppView search within seconds), re-activated (returns)**, and a second listing under the same DID publishes independently (pausing one does not touch the other). Paused/deleted listings do **not** reappear in search (ghost-listing fix). | |
| MT-96 | P1 | Services / Taxonomy | Custom capability excluded from generic AI search | A **custom** (provider-specific) capability is reachable by exact NSID/URI/profile-browse but is **NOT** returned by generic intent search (`searchCapabilities`) or offered by the AI for an open-ended ask. A **subject-scoped** canonical capability (e.g. appointment_status) is likewise never generic-routed. | |
| MT-97 | P1 | People | Contact vs Relation separation | A **service contact** (e.g. a depot/clinic added for a grant) appears under **People → Contacts** but **NOT** under **Relations**. A person with a relationship hint (e.g. "daughter") or vault evidence DOES appear under Relations. | |
...
| Unlisted share/QR/invite | Link-paste invocation SHIPPED + live-verified 2026-06-10 (paste the `at://` listing URI in chat; the resolved listing is authoritative for schema+name). QR/invite UX still not built. | Test the link-paste path (MT-78); do not claim QR sharing. |
| Known-only service invocation | SHIPPED + live-verified 2026-06-10: provider offer (`/v1/service/offer`, contact-gated) → grant rides the contact → invoke via chat "Ask <contact> …". No dedicated browse UI. | Test via MRS-11; do not claim a private-service browse/marketplace UI. |
...
| MsgBox WS staleness after long idle | Found live 2026-06-10: after ~3.5h idle the app's relay WebSocket was dead and did NOT auto-reconnect — inbound agent claims/D2D silently lost; task delegation timed out (honest in-chat timeout message shown). App reload restores. | Before agent/D2D test runs after long idle, reload the app OR verify relay connectivity first. Fix candidate: WS heartbeat + auto-reconnect. |

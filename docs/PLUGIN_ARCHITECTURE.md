...
Status: design plus partial P0 substrate implementation. The protocol types, persistence repositories, authorization boundaries, and runner claim/complete lane exist, but plugins are **not yet available end-to-end**: as of 2026-09-13 both hosts have the install/consent/uninstall routes and screens (`/v1/plugins/install/*`, `apps/mobile/app/plugins.tsx`), a wired repo-proof verifier (one shared chain, `@dina/home-node/repo_proof_chain`; server `@dina/net-node`, phone `@dina/net-expo/repo_proof` — bundle-proven, device run owed), pending-install pairing bound inside Core (`completePairing`, §15.3), the abandoned-install sweep, and a first-party door for the shipped manifests (commerce packs, country packs). The owner-side dispatch producer exists as of 2026-09-13 (`packages/core/src/plugins/invoke.ts`, `POST /v1/plugins/invoke`; one delegation task on the plugin lane, `queued` under a charged grant or `pending_approval` for the owner's card; the workflow approve/cancel verbs record the decision and mint `plugin_grant`s; the phone's approval inbox renders the §15.5 card from the pinned envelope). Chat routing into plugins (§6) exists: Brain's `list_plugin_capabilities` / `invoke_plugin` tools call `GET /v1/plugins/tool-capabilities` and `POST /v1/plugins/tool-invoke`, the same producer; on the server split a `brain` caller cannot decide the card (the phone's one VM is a typed-import boundary, per CLAUDE.md). A completed invocation's validated result renders through the capability's own pinned CARD TEMPLATE as of 2026-09-15 (`packages/protocol/src/plugins/card_template.ts` + `packages/core/src/plugins/result_card.ts`): the manifest declares the layout and its literal words, a SLOT (`"{field}"`, a whole string) names a field of the same capability's `result_schema`, and the envelope pins the template at enqueue (`card_snapshot`) so an update cannot re-frame an answer in flight. The render runs `validateCardSpec` untrusted and keeps only `blocks` — badges, links, media and the card's own source/freshness frame are Dina's, never a publisher's. A capability that declares no template still answers on the bounded `label: value` floor. The Core-owned context projector exists as of 2026-09-15 (`packages/core/src/plugins/context_projection.ts` + `context_templates.ts` + `context_sources.ts`): the producer no longer takes a `context` at all — a caller names a SUBJECT (`{contactDid?, documentDigest?}`) and Core projects the facts itself, through a first-party versioned table keyed by (category, action class), scrubbed per value, bounded by `max_context_items`, with categories + counts + a payload hash on the audit line. Its sources today are the three stores Dina owns for trade (`business_registry`, `address`, `contact`, registered from `installCommerceRuntime`); a consented category with no source projects nothing and is reported as unsourced. The same change wired the §11 persona ring, which had been an injectable policy no composition root ever passed. Production still lacks the advisory worker and the plugin SDK workflow. Grounded against the shipping TypeScript stack (`packages/core`, `packages/brain`, `packages/protocol`, `appview/`, `apps/mobile`). Every "exists today" claim carries a file reference.
...
**Scope: plugins are not agents.** A plugin is a bounded capability the owner installs *into* Dina — declared schemas, locally derived risk, consented scope, dispatched work. An agent is an external reasoning system acting *under Dina's authority*, paired under the agent rules with its own grant surface and per-request gating. This document specifies the plugin substrate only: workloads that need interactive data access or open-ended tool use are agents and pair as agents (§19). A broader, framework-independent control-plane contract sitting above both is a possible future layer with its own document — an optional expansion, not a prerequisite for anything here.
...
- **Manifest**: the plugin's contract, carried whole inside an immutable, content-addressed release record (`com.dinakernel.plugin.release`, one per version; a mutable `com.dinakernel.plugin.identity` record points at the current one — §5); the release CID *is* the hash, so content addressing comes free from ATProto. The CID pins a *version's content* and nothing more: identity is the versionless pair `(publisherDid, plugin_id)`, and authenticity is a repo proof (§5, rule 5). Nothing ever validates against a live record.
...
- **Kind** (runner mode): which lanes the code may serve: `tool`, `provider`, `ingest`, `notify` (§9). Declared per capability in the manifest (`kinds`, §5), validated for legal combinations, rendered at consent, and inside the scope hash (§8.1) — Core authorizes against a consented declaration, never an inferred one.
...
## 5. The manifest and its records: `plugin.identity` + `plugin.release`

Two record types, because ATProto repositories keep only *current* records: an update replaces the old version in the repo, and a delete can leave nothing behind. A single mutable manifest record would therefore make every previously pinned CID unverifiable to a new recipient — breaking the invite bootstrap (§10.1) on any version skew, since repo proof can only prove what the current repo contains. So:

- **`com.dinakernel.plugin.release`** — immutable, one record per version. Immutability is *enforced, not assumed*: ATProto lets a publisher overwrite a record at the same rkey, so the release rkey is **content-derived**, byte-for-byte frozen with the lexicons: rkey = the lowercase base32 encoding of the SHA-256 digest over the record's canonical dag-cbor encoding — the same bytes ATProto's own record CID hashes, so the rkey is a pure function of the CID, checkable by any holder of the record with no bespoke canonicalization scheme to get wrong (52 chars, within rkey charset and length limits; conformance vectors land beside the protocol's other frozen vectors in `packages/protocol/conformance/`). Every verifier (the on-node installer and AppView alike) checks `rkey == f(cid)` before accepting anything. An overwritten release fails that check everywhere, with no reliance on AppView having watched the firehose; direct install (which bypasses AppView by design) gets the same guarantee. It carries the complete manifest below. Installs and sessions pin a release URI + CID; because old releases remain present in the publisher's current repo, a new recipient can still repo-proof-verify an older pin against the current signed repo state. Version strings are labels for humans: `(plugin_id, version)` uniqueness is revalidated at ingest and cross-checked at install, but version-specific evidence — advisory `affected_cids`, PeerLens version signals (§13) — keys on CIDs, never on the version string, so a reused label cannot launder a different build.
- **`com.dinakernel.plugin.identity`** — mutable, stable rkey = `plugin_id` (multi-plugin publishers follow the multi-listing rkey precedent, `packages/brain/src/service/service_publisher.ts:31-42`). It points at the current release and carries nothing that needs pinning. Its shape freezes with the lexicons: `{plugin_id, current: {uri, cid, version}}`, with five pointer invariants the verifier enforces — the pointed release lives in the *same publisher repo*, in the release collection, its `plugin_id` equals the identity record's rkey, the record at `current.uri` matches `current.cid`, and `current.version` equals the release's own `version` (the pointer must not lie about the label discovery UI will render). A pointer failing any of the five is treated as no pointer at all.

Deleting a release is allowed and bounded: nodes that installed it verified and stored the snapshot locally, so **existing installs and running sessions never depend on the public record surviving** — a publisher cannot break them by deleting history. Only a *new* recipient invited to a deleted release fails bootstrap, with a clear error ("this version is no longer published"), per rule 5's hard-fail UX.

The manifest, carried in full inside each release:

```jsonc
{
  "$type": "com.dinakernel.plugin.release",
  "plugin_id": "com.acme.battleship",        // = the identity record's rkey; identity is
  ...
  "icon": { "$blob": "..." },
  "homepage": "...", "source_url": "...",
  "min_interpreter": 1,                       // interpreted: node refuses constructs it doesn't know
  "min_plugin_protocol": 1,                   // runner: envelope/lifecycle contract version
  "required_features": ["notify"],            // declarations only ADD: the installer derives the
                                              //   real requirement set from manifest structure and
                                              //   unions this in (§14); node lacking any → clear
                                              //   "needs a newer Dina" — never a silently dead lane

  "execution": { "mode": "interpreted" },     // or {"mode":"runner","runtime":{
                                              //   "hosted_endpoint": "...",
                                              //   "issuer": {"did":"...","key":"..."},  // hosted: signs instance certs (§14)
                                              //   "self_host": {"npm":"...","docker":"..."},
                                              //   "artifacts": {"npm_integrity":"...","image_digest":"...",
                                              //     "source_commit":"...","provenance":"..."}}}  // evidence tiers, §12
  ...
  "capabilities": [{
    ...
    // runner mode only:
    "kinds": ["tool"],                          // lanes this capability may serve (§9):
                                                //   tool|provider|ingest|notify. Legal combos
                                                //   validated (§5.3); consent-rendered; hashed (§8.1)
    "effects": { "idempotency": "unsupported" },// or "supported": provider dedupes on idempotency_key
                                                //   for at least Core's retry window — gates retries (§9.1)
...
3. **Ingest revalidates at the trust boundary.** The AppView handler re-derives classes, caps sizes, rejects banned categories, and validates `kinds` combinations (ingest/notify are runner-only; `provider` requires `query` interaction; session capabilities carry no kinds) — the on-node installer runs the identical validation, mirroring `appview/src/ingester/handlers/service-profile.ts:106-125`. Companionship/emotional-intimacy capabilities are banned at ingest AND at install (Anti-Her, enforced twice). Single-"player" session capabilities are rejected: a session requires a peer DID that is an owner-selected known contact, explicitly accepted (§17.2).
4. **Install-time structural caps** (schema bombs are a real vector): manifest ≤ 256 KB, schema depth ≤ 8, no recursive `$ref`, ≤ 64 states, ≤ 32 move types, ≤ 16 transitions per state, ≤ 32 ops per transition. Numbers tunable; the existence of hard caps is not. Uniqueness rides alongside the caps: duplicate capability ids, duplicate entries within `kinds`, duplicate state or move ids, and ambiguous `(state, move)` transition pairs are rejections, never first-match-wins — grants, routing, envelopes, and scope hashes all identify a capability by its id string, and two definitions behind one string is how independent components come to silently disagree.
...
- **Owner-local routing is opt-in**: with consent, a runner-mode tool capability's `intent_phrases` join the classifier's injected list, the same mechanism as `ROUTABLE_CAPABILITY_LINES` (`packages/brain/src/reasoning/intent_classifier.ts:89-91`), as a runtime-sourced second list with a new source value `installed_plugins`. The toggle lives on the consent screen; off by default for HIGH capabilities. Phrases are consent content, not cosmetics: they render verbatim on the consent card, they sit inside the approved-scope hash (§8.1) so a phrase change is a re-consent diff, and when an utterance matches more than one claimant — two plugins, or a plugin and a service — routing never silently picks: a first-party picker asks, the same stop-and-ask rule as multi-install routing (§15). A hijacked route is further bounded by the params-egress gate (§11, point 5): misrouted sensitive params card before anything leaves.
...
**Standing approvals** clone the `service_grants` shape (`packages/core/src/service/service_grant_repository.ts:20-38`) into `plugin_grants`, keyed `(install_id, capability, approved_scope_hash)`, with *Once / 24 hours / Until revoked* offered on the card (the vault-read card's approve vs approve-once split, generalized). One restriction on the third option: for HIGH-class capabilities (`booking`/`write`/`agentic`) an *unconstrained* "Until revoked" is never offered — "always allow booking" would otherwise mean any date, any provider, any amount the schema admits. Standing approval there must carry at least one built-in constraint on the grant row: a max invocation count, a time window, a resource/recipient allowlist, or a value cap where params carry amounts. The first-N rule cards invocations 1–3; constraints are what govern invocation 4 onward. The full predicate language is deferred, but the grant row carries the optional `constraints` object from day one, so the vocabulary can grow without re-keying grants. And constraints are enforceable or they are theater, so three mechanics ship with the object: constraint schemas are **versioned**, and an unknown constraint type **fails closed** — the grant simply doesn't match and the invocation cards; count/value/rate consumption is an **atomic check-and-consume at dispatch** (one transaction, never read-then-update, so concurrent dispatches cannot jointly exceed a limit); and qualifying constraints have enforced ceilings — a `count` of a billion is not a constraint, and a nominally-constrained grant must not recreate the blank check it exists to prevent. Consumption keys on the **logical execution**, not the dispatch: a use is consumed atomically when an `execution_id` is first dispatched, and idempotent lease-recovery retries under the same `execution_id` never consume a second use — the first-N card rule counts the same way. Reservations release only when a task provably never executed (never claimed, or terminalized `stale_authority` before any claim); an `outcome_unknown` ending stays consumed, conservative in the constraint's favor. The scope hash is a canonical digest of the capability's consent-relevant fields, specified field-by-field in §8.1. Keying on it makes update policy self-enforcing: a same-scope update leaves the hash unchanged and grants survive; scope growth changes the hash, nothing matches, and re-consent happens structurally rather than because a rule remembered to fire. Execution always validates against `current_cid`, pinned separately on the install row. Uninstall and device-revoke cascade grants (`revokeForAgent` precedent, `registry.ts:288-297`).
...
The hash IS the re-consent boundary, so its contents are a launch gate, not an open decision. Per capability, the digest is SHA-256 over JCS-style canonical JSON (sorted keys, no insignificant whitespace), with one normalization JCS does not perform: **set-like arrays are deduplicated and sorted** (Unicode code-point order) before hashing — `kinds`, `data_scope.categories`, personas, `network_domains`, `ops_used`, `intent_phrases` — because JCS sorts object keys but preserves array order, and `["tool","provider"]` versus `["provider","tool"]` must not manufacture a re-consent. Normalization is not hash-private: the normalized form is the **stored form** — what ingest validates, what the registry keeps, what consent renders, and what runtime consumes. `intent_phrases` ordering in a classifier prompt is behavior, so two manifests that hash alike must also *run* alike; hashing one representation while executing another would let identical consent cover divergent behavior. The digest covers:

**In the hash** — anything that changes what the owner agreed to:

- execution mode; capability id; `action_class`; `privacy_class`; `kinds` (which lanes the capability may serve — an update that adds `notify` to a tool capability is new consent, §9); `effects.idempotency` (it changes retry behavior the owner is agreeing to, §9.1)
- `params_schema` and `result_schema`
- `data_scope` (categories, personas, `max_context_items`)
- `network_domains` and the runtime identity: `hosted_endpoint`, the runtime issuer (`runtime.issuer` DID + key, §14), and any pinned artifact identity (`runtime.artifacts`, §12) — a new issuer or a new artifact means a different party or different code receiving the owner's data, which is consent, not behavior
- `config_schema` (a new field asking the owner for more input is consent-relevant)
- `intent_phrases` (they change *when* the plugin gets invoked; a phrase change appears verbatim in the re-consent diff, and multi-claimant collisions route through a picker, §6)
...
|---|---|---|
| P0 | `/v1/workflow/tasks/claim`, `/:id/heartbeat`, `/:id/progress`, `/:id/complete`, `/:id/fail`, `/healthz` | the six claim-time checks (§9.1): lane, active, kind, non-revoked, current scope hash, current config revision — plus per-claim `claim_id` on every subsequent verb |
...
Intent classifier (opt-in injection, §6) → `evaluatePluginIntent` → SAFE runs silent *only if the params clear egress* (§11 point 5), MODERATE/HIGH raises `InlinePluginApprovalCard` → brain assembles minimal context (§11), egress-checked → `WorkflowTask{kind: delegation, requested_runner: "plugin:<install_id>", payload: {install_id, capability_id, params, context, manifest_cid, approved_scope_hash, schema_snapshot, config_revision, execution_id, idempotency_key, attempt}}` — the pinned identity and consent fields are immutable, set at enqueue, and are what the six claim-time checks verify against; without them persisted on the task, the stale-authority guard is unimplementable. `attempt` is the deliberate exception: it advances by one on every claim (a lease reclaim IS a new attempt — an attempt counter frozen at enqueue counts nothing), while `execution_id` and `idempotency_key` stay fixed across attempts. The execution triple exists because side effects outlive leases: a runner can book, crash before completing, lose the lease, and watch the task get re-dispatched. And because a custom capability's declared class is a consent label, not proof (§8), retry safety cannot key off it — a runner that declared `read` can still mutate the world. So the retry rule trusts nothing: **once a runner task has been claimed, it is never automatically re-dispatched — any capability, any declared class — unless the capability declares `effects.idempotency: "supported"`** in the manifest (§5), a field inside the scope hash, so retry behavior is part of what the owner consented to. A task that was never claimed never executed and may re-enqueue freely. Where the lost work was *declared* effectful (`booking`/`write`/`agentic`), it parks as **`outcome_unknown`**, a first-class terminal status (§9.5); declared-read work ends as plain `failed` — the declared class governs the owner-facing narrative, never the retry decision. And even `"supported"` buys a bounded budget, not a loop: Core owns the retry contract — proposed ≤ 3 attempts, exponential backoff, all inside a retry window of 24 h from first dispatch — and `"supported"` is a declaration that the provider deduplicates on `idempotency_key` for **at least that window** (§5); a provider that forgets the key sooner would duplicate the action on a late retry, which is exactly the lie the declaration exists to make attributable. Budget exhaustion without a terminal report ends like any vanished execution: `outcome_unknown` if declared-effectful, `failed` otherwise → instance claims over signed MsgBox RPC (`msgbox_handlers.ts:245`), executes, completes → result validated against the **pinned** result schema (nonconforming = task failure, counted against plugin health) → CardSpec render in untrusted mode (badge blocks stripped, no external URLs — the sole sanctioned exit is the first-party Open-link card, §15; `card-spec.ts:281`).

**Claim-lane hardening (required before any third-party runner exists):** today's claim SQL lets a named filter also take *untagged* tasks (back-compat clause, `repository.ts:582-588`). For `plugin` callers the server ignores the client-sent `runner_filter` entirely and forces exact-match on the lane registered to that instance DID, the way `dina.local` is exact-match-only and HTTP-claim-forbidden (`routes/workflow.ts:216-220`). Lane assignment derives from the dynamic registry row, never from the caller. And the lane match is only the first of six claim-time checks, all server-side: exact lane; `install.status === active`; the capability's declared-and-consented `kinds` covers this task's lane (§5); the device is non-revoked; the task's pinned scope hash equals the install's *current* approved hash — closing the stale-authority hole where a task queued under old consent gets claimed after the owner changed it; and the task's pinned `config_revision` equals the install's current one — closing the approve-then-reconfigure hole where the owner approves a booking and a config edit then redirects it to a different account, destination, or tenant (§14). The two pin checks *terminalize*, not merely reject: a scope-hash or config-revision mismatch is irreversible — the task's pins are immutable and the install's current values are not coming back — so the guard atomically moves the task to `failed` with typed reason `stale_authority` and an owner-visible explanation ("cancelled: settings changed after this was queued"), offering a fresh re-run through the gate under current pins. Leaving it queued would let the claim loop re-select the same dead oldest row forever, starving valid work behind it. Transient conditions are different: pause merely removes the lane from selection and needs no producer-side cooperation — in-flight leases may complete, queued tasks wait, new claims stop at the guard. This is a **launch gate, enforced in Core**: no third-party runner pairs until it ships server-side. SDK behavior is irrelevant to the invariant; a malicious runner is assumed to speak raw RPC.

**Lease ownership is a token, not a status.** Today's repository lets any nonterminal task be completed without checking the current lease (`packages/core/src/workflow/repository.ts:701` region) — survivable for cooperative runners, unacceptable for effectful plugins: an old execution could complete *after* the task was re-claimed, applying a stale result over a newer attempt's authority. So every claim mints a random `claim_id`; heartbeat, progress, complete, and fail all require it; and terminal transitions are a compare-and-swap on `(task_id, claim_id, running)`. A stale execution's completion loses the CAS and is recorded as a late report — evidence, never a result. `attempt` increments with each new `claim_id`; `idempotency_key` never does.

### 9.2 `provider` — the plugin backs the owner's own listings

Exists today under another name: service capabilities already route by `requestedRunner = mcpServer || 'dina.local'` (`service_handler.ts:494-500`). A provider-kind plugin registers `plugin:<install_id>` as the binding for chosen capabilities in the owner's listing config (`service-settings.tsx` gains a "Backed by" picker). Inbound `service.query` → ServiceHandler (auto/review unchanged) → plugin lane → Response Bridge returns the schema-validated `service.response` (`workflow/service.ts:443-447`). ChairMaker ships one plugin; every customer gets an order-status service without writing code. All existing gates apply because the plugin sits *behind* the listing, invisible to peers — **and the provider task carries the identical pinned plugin envelope as a tool task** (`install_id, capability_id, manifest_cid, approved_scope_hash, schema_snapshot, config_revision, execution_id, idempotency_key, attempt`, §9.1), verified by the same six claim-time checks and claim-token discipline (§9.0, §9.1). Stated explicitly because "all existing gates apply" would otherwise make the provider lane the soft path: a provider task with no pinned envelope would skip the stale-authority and active-install checks that the tool lane enforces. Same envelope, same guard, no exceptions.
...
### 9.5 `outcome_unknown` is a workflow state, not a phrase

Retry (§9.1) and revoke (§14) both end in "outcome unknown," so it exists in the state machine (`packages/core/src/workflow/domain.ts:19` is the shared status union every workflow consumer parses), specified before P0, not improvised at the first crash:

- **Stored value**: `outcome_unknown`, terminal. Nothing transitions out of it; reconciliation acts *beside* it in the decision log, never mutates it back to a live state.
- **Legal entries**: `running → outcome_unknown`, for a *declared-effectful* task (`booking`/`write`/`agentic` without `effects.idempotency: "supported"`) whose execution started but which produced no terminal report from the executing instance. Lease loss and instance revocation are two of five such paths; **owner cancellation of a running task, deadline/TTL expiry mid-run, and Core shutdown are the same epistemic situation** — Dina cannot know whether the external action occurred, and naming those `cancelled` or `failed` would claim knowledge it doesn't have. Cancelling a running effectful task says so in the UI: "stop tracking — the booking may already have happened." (The existing cancellation path terminalizes any nonterminal task directly, `packages/core/src/workflow/repository.ts:761`; claimed effectful plugin tasks must be special-cased out of it.) Declared-non-effectful tasks keep the existing endings (`failed`/`cancelled`) — read work that died is not a mystery, and routing it here would dilute the state until nobody respects it. The asymmetry with retry is deliberate: retry safety trusts no declaration (nothing re-dispatches post-claim without supported idempotency, §9.1), while this *classification* follows the declared class. A capability that lies `read` and mutates gets no duplicate effect out of Dina; what it corrupts is its own failure narrative, which is §17's honesty-clause territory.
- **Reconciliation actions** (owner-facing, from the card and Activity): *confirmed it happened* (recorded; any late report linked as evidence), *confirmed it didn't* (recorded; optionally re-dispatch as a **new** task with a fresh `execution_id` — same `idempotency_key` only where the capability supports it), or *leave unresolved*. All three are decision-log entries; none rewrites the task row.
- **Retention**: the task row and its reconciliation evidence are exempt from normal sweeping until the owner resolves or an extended TTL passes — an unresolved external effect is precisely the thing that must not be garbage-collected first.
- **Migration reality**: the status union is parsed by every consumer of the workflow tables, so this is a schema + validator change across all of them, which is why it is a P0 gate (§20) and not a follow-up.
...
- **Invite** rides the existing offer/grant machinery (`service_offer_events.ts`, `grant_request_events.ts`). Alonso picks a contact and a plugin; Sancho receives an invite card. The invite carries `(publisherDid, plugin_id, release_uri, manifest_cid)` — the release URI matters because the mutable identity pointer may have moved on since this version (§5) — which makes it the third reach-by-reference channel (§6): it must work when Sancho has never seen the plugin. Not-installed path: repo-proof verification of that exact snapshot → a combined install-consent + session-accept card ("Alonso invited you to Battleship — install and play?") → grant minted only after both. Already-installed path: if Sancho's `current_cid` differs from the invite's pin, the accept card says so and the session runs the pinned snapshot on both sides — verified and consented locally like any install — so version alignment falls out of the pin instead of being negotiated. Deleted-release semantics are bounded (§5): if the publisher removed that release, only *bootstrap* fails — clearly, "this version is no longer published" — while every node that already installed it holds the verified snapshot locally, so existing installs and running sessions never depend on the public record surviving.
...
5  dina-plugin publish               putRecord plugin.release (immutable, one per version)
                                     + move the plugin.identity pointer → Jetstream →
                                     AppView ingest-validates (§5.3) → marketplace-
                                     searchable. Installers verify authorship by repo
                                     proof (§5 rule 5), never by index say-so
...
7  ship an update                    new plugin.release, new CID; the identity pointer
                                     moves. Old releases stay in the repo, so pinned
                                     sessions and invites keep verifying. Installed
                                     Dinas see the diff; §14 governs application
...
Two refinements keep that honest. **Runtime evidence tiers** (`runtime.artifacts`, §5): a runner manifest may pin what it truthfully can — npm integrity hash, container image digest, source commit, a provenance statement — and instances report their runtime version/digest with every claim and result. Dina still cannot prove hosted behavior; what evidence buys is *legibility*. Consent and PeerLens can then distinguish a **pinned self-hosted artifact** from a **signed vendor deployment** from a **mutable opaque hosted service**, and say in plain words which of the three the owner is trusting. Evidence supplements PeerLens; it never substitutes for it.

**Private distribution is signed distribution.** The public repo proof is one trust anchor, not the only one. A production install verifies against exactly one of: (a) a public `plugin.release`, by repo proof; (b) a private organization registry record, signed by an org key the owner has explicitly approved — the enterprise path; or (c) a local publisher key the owner explicitly trusts at install time, including the owner authoring a plugin and declaring themselves publisher. All three run through the same verifier module (§5 rule 5) with an explicit, named trust anchor; "merely signed" by an unknown key counts for nothing. *Unsigned* remains debug-builds-only (§20). Private never means unsigned.
...
**Install, runner (self-host)**: detail → *Install* mints a `pending` install row with an expiry → Dina shows the setup code → `dina-plugin serve --setup-code dina1:…` → pairing (role `plugin` fixed at initiate) → consent → **activation, the single atomic commit point**: the device attaches to the install, per-capability egress scopes + sharing-policy backstop rows + lane registration land, status flips to active. Everything before activation is reversible: a cancelled consent auto-revokes any device paired during the ceremony, and the abandoned-install sweeper expires stale pendings **and revokes their devices too** — the dangerous case is a pairing that completed but whose consent was never confirmed, leaving a paired-but-not-activated device; the sweeper must `revokeDeviceDurable` that device, not just delete the pending row, or a live plugin device outlives the abandoned install. No orphan plugin devices survive, whether the install was cancelled or simply abandoned. Hosted path: the handshake must *bind* the instance to the plugin AND to this owner's install, not merely name a key. At install start the owner's Dina mints a single-use `install_nonce` and passes it through the handshake; the publisher signs an **instance certificate** over `(instance_pubkey, plugin_id, manifest CID, owner_did, install_nonce, expiry)`; the instance proves possession of the key through the pairing challenge; Dina completes pairing only if all six fields verify and the certificate's signer equals the manifest's declared **runtime issuer** (`runtime.issuer`, §5) — not the repo DID: on a community PDS the repository signing key is held by the PDS, so the publisher's application cannot sign with it. The repo-signed manifest authorizes the issuer; the issuer signs per-install certificates; and because the issuer sits inside the approved-scope hash (§8.1), rotating it is a manifest update *and* a re-consent event — a new issuer is a new party able to instantiate the plugin. Without the owner/nonce binding, a certificate minted for tenant A could complete a pairing with owner B and silently route B's payloads to what the vendor considers A's account. A deep link that merely carries a pubkey proves nothing and pairs nothing. Self-host keeps its human binding: the owner ran the process and typed the code, which remains the only secret.

**Configure**: `config_schema` renders the form; values stored via the per-rkey config pattern (`service_config_repository.ts`), hot-reloaded through a `config_changed`-style event (`config_event_channel.ts`). The store is Core-owned, never the plugin vault (§5 rule 6) — which is what lets the form render and save while the install is paused and its vault unmounted. Every save bumps a monotonic `config_revision`; the revision in force at enqueue rides the immutable task envelope, and claim check six (§9.1) refuses to dispatch work under settings the owner didn't see when it was approved — hot reload serves *future* tasks, never in-flight authority. That boundary holds *through* execution, not just at claim: the claim response carries a task-scoped snapshot of the pinned revision's values, the SDK hands exactly that snapshot to the handler (`ctx.config`, §16), and a `config_changed` event applies to future claims only — a claimed task never sees settings newer than its approval. Editing config while approved tasks are in flight surfaces the choice — let them finish under the settings they were approved with, or cancel them — never a silent mid-flight swap.

**Update**: a new CID on the same install row, governed by two separate boundaries. The approved-scope hash (§8.1) decides **grant survival**: unchanged hash → standing approvals survive untouched; changed hash → a re-consent card shows only the diff, the old pin stays live, and no grant matches the new scope until the owner approves. Grant continuity is structural, not remembered. **Application** of the new CID is a separate policy, because the scope hash guards consent, not behavioral trust — a same-scope interpreted update can still rewrite how a game behaves. The `behavior_hash` (over the machine wiring §8.1 keeps out of consent) makes functional changes visible: unchanged behavior + unchanged scope → applies silently on next instance restart (runner) or next new session (interpreted; in-flight sessions always finish on their pinned CID); changed behavior hash → application follows the owner's auto-update preference with a trust-ring default (Verified+Actioned auto-applies; below that, a low-key Activity prompt), and every application lands in the Activity digest with version and what changed. A third boundary handles cosmetics: a changed **`presentation_hash`** (display name, description, instructions — §8.1) never blocks a functional update and never gates grants, but a before/after entry **always** appears in Activity when it lands, so a "silent" update is never silent about a rename or an instruction rewrite. "Silent application" means silent to the *machinery*, never invisible in the log. **Trust ring is not sufficient on its own, though:** a behavior change to any capability whose `action_class` is `booking`, `write`, or `agentic`, or whose `privacy_class` is `sensitive`/`regulated`, **always requires explicit approval** — even from a Verified+Actioned publisher. Auto-apply is reserved for behavior changes on read/quote, public/personal capabilities, where a silent rules tweak cannot spend money, mutate the world, or touch sensitive data. High-consequence capabilities never move behavior silently, whatever the ring. Runner mode carries the honest asterisk: its code lives out of band, so a vendor can change server behavior with no manifest update at all — update policy governs the contract there; PeerLens governs the behavior. Compatibility gates both install and update, and the requirement set is **derived, never merely declared** — the same move as risk classification (§5, rule 1): the installer computes what the manifest actually needs from its structure (declared `kinds`, card blocks, ops, envelope features), unions in the publisher's `required_features`, and fails closed on any field or feature it doesn't recognize. A publisher who declares `kinds: ["notify"]` but omits `notify` from `required_features` gets the same refusal — the gate keys on what the manifest *uses*, so under-declaring cannot bypass it. A release whose `min_plugin_protocol` exceeds the node's, or whose derived requirements name behavior the node hasn't shipped (a `notify` manifest on a node that delivers notify in P3), fails with a clear "needs a newer Dina" refusal. A consented kind the node cannot actually deliver must be unreachable, not silently dead.

**Pause**: lane stops accepting / invites stop; pairing, config, vault survive.

**Uninstall / revoke**: interpreted → registry removal + plugin-vault deletion + session termination + grant cascade. Runner → `revokeDeviceDurable` (durable-first, cascades grants, idempotent, audits `device_revoked`, `registry.ts:262-300`) + the same cleanup + optional ingest purge. One tap, immediate, no publisher cooperation. **Immediate means immediate, including in-flight** (this is the distinction from pause, §9.1, where running leases finish): revoke fails any running task for that instance at once, and a completion posted by a revoked instance is never *applied* — no card, no state change, no result into context. But revoke's honesty has a boundary: Dina stops its own side instantly; it cannot undo an external action the runner already performed. An *effectful* task in flight at revoke therefore lands in **`outcome_unknown`** (§9.5) — revoked locally, external outcome unknown, reconciliation required — surfaced to the owner as "a booking may have completed before revoke"; a non-effectful in-flight task simply becomes `failed`, because read work that died is not a mystery worth tracking. And a late completion report from a revoked instance is not discarded either: it is retained as **reconciliation evidence** in the decision log, visibly marked as unverified post-revoke input, because hiding it would make the owner less informed, not safer. Never applied, never hidden.
...
servePlugin({
  ...
  handlers: {
    'com.acme.flight_watch': async (params, ctx) => {
      ...
      // ctx.config   – task-scoped config snapshot: the revision this task
      //                was approved under, never hot-reloaded mid-task (§14)
...
| Publisher rewrites or deletes history (a pinned release vanishes) | Immutable `plugin.release` records, one per version; installs and sessions run from the verified local snapshot; only new-recipient bootstrap of a deleted release fails, with a clear error (§5, §10.1) | net-new lexicon split |
| Release overwritten in place / version-string reuse for different content | Content-derived release rkey; every verifier checks `rkey == hash(body)` — no AppView dependence, direct install included; version evidence keys on CIDs, never version strings (§5) | net-new verifier rule |
| Wrong runner paired via hosted deep link / cross-tenant confusion | Instance certificate bound to owner + install: (instance_pubkey, plugin_id, CID, owner_did, install_nonce, expiry), single-use nonce; signer must equal the manifest's declared runtime issuer, itself inside the scope hash (§14) | net-new binding |
...
| Standing approval as a blank check ("always allow booking" = any date, any amount) | HIGH-class standing grants must carry built-in constraints (count, window, resource/recipient, value cap); unconstrained Until-revoked is not offered for booking/write/agentic (§8) | net-new constraint object |
...
| Crash or lease loss duplicates an external side effect | `execution_id` + `idempotency_key` + `attempt` in the envelope; NO runner task auto-retries post-claim without `effects.idempotency: "supported"` — declared class is not trusted for retry safety (§9.1); declared-effectful losses park as `outcome_unknown` (§9.5) | net-new envelope fields |
| Stale execution completes after the task was re-claimed | Per-claim random `claim_id` required on heartbeat/progress/complete/fail; terminal transitions CAS on `(task_id, claim_id, running)`; the loser's report is recorded as evidence, never applied (§9.1) | net-new lease token |
| Approve-then-reconfigure redirects an approved action | `config_revision` pinned in the immutable envelope; claim check six refuses dispatch on pre-claim drift; post-claim, execution runs on a task-scoped snapshot and hot reload serves future claims only (§9.1, §14) | net-new claim check |
| Concurrent dispatches jointly exceed a count/value constraint | Atomic check-and-consume at dispatch; versioned constraint schemas; unknown constraint types fail closed to a card (§8) | net-new grant mechanics |
...
| Semantic route hijack via `intent_phrases` | Phrases are consent content: verbatim on the consent card, inside the scope hash (change ⇒ re-consent); multi-claimant matches go to a first-party picker, never silent routing; misroutes bounded by the params-egress card (§6, §11) | net-new routing rule |
...
| External action lands after revoke | A revoked instance's completion is never applied and never hidden: retained as reconciliation evidence, owner sees "external outcome may have occurred" (§14) | net-new reconciliation state |
...
| Malicious update via the provider lane | Provider tasks carry the identical pinned envelope + six claim-time checks as tool tasks (§9.2) — no soft path | net-new, closes the bypass |

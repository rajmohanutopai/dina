...
"${CLAUDE_PLUGIN_ROOT}/bin/dina-cli" audit` (append `--limit $ARGUMENTS` if the
user gave a number) and summarize the recent entries, most recent first: which
actions Dina classified and how — allowed, flagged for your approval, or
blocked — with the reason where one is given.

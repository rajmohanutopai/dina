...
def test_managed_install_is_pipless_and_atomic(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setenv("DINA_SETUP_CLI_SPEC", "dina-agent==0.20.11")
...
def test_production_wheels_are_exactly_pinned() -> None:
    ...
    assert bootstrap.REQUIRED_CLI == (0, 20, 11)
    assert bootstrap.DINA_WHEEL_URL.endswith("dina_agent-0.20.11-py3-none-any.whl")
...
def test_development_mode_rediscovers_a_newer_compatible_cli(
    ...
    monkeypatch.setenv("DINA_PLUGIN_DEV_MODE", "1")
    monkeypatch.setenv("DINA_SETUP_RUNTIME_DIR", str(runtime))
    monkeypatch.setattr(bootstrap, "_cli_version", lambda _path: (0, 20, 12))

    assert bootstrap._find_compatible_cli() == (managed, (0, 20, 12))

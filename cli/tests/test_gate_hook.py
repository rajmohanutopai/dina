...
SUPERVISOR = Path(__file__).resolve().parents[1] / "claude-plugin" / "dina" / "bin" / "dina-gate"


def _cfg():
    from dina_cli.config import Config

    return Config(core_url="http://localhost:8100", timeout=5.0, device_name="test-device")
    ...
    *,
    ...
    if inject_session:
        try:
            ...
            pass
    runner = CliRunner()
    with patch("dina_cli.main.DinaClient") as MockCls, patch(
        "dina_cli.main.load_config"
    ) as mock_load, patch("signal.signal"), patch("signal.alarm"):
        ...
        return runner.invoke(cli, ["gate-hook"], input=stdin_text)
...
def test_allow_exits_0():
    r = _run_gate(json.dumps({"tool_name": "Read", "tool_input": {"file_path": "a.ts"}}),
                  gate_return={"action": "code_read", "risk": "SAFE", "outcome": "allow"})
    assert r.exit_code == 0


def test_deny_exits_2():
    r = _run_gate(json.dumps({"tool_name": "Bash", "tool_input": {"command": "cat keyfile"}}),
                  gate_return={"action": "secret_read", "risk": "BLOCKED",
                               "outcome": "deny", "reason": "reads a protected path"})
    assert r.exit_code == 2


def test_approval_required_asks():
    r = _run_gate(json.dumps({"tool_name": "Bash", "tool_input": {"command": "git push"}}),
                  gate_return={"action": "vcs_push", "risk": "MODERATE",
                               "outcome": "approval_required", "reason": "pushes to a remote"})
...
def test_high_approval_required_blocks_until_dina_approval():
    r = _run_gate(
        json.dumps({"tool_name": "Bash", "tool_input": {"command": "git reset --hard"}}),
        gate_return={
...
def test_unreachable_core_fails_closed():
    ...
    r = _run_gate(json.dumps({"tool_name": "Read", "tool_input": {}}),
...
def test_missing_tool_name_fails_closed():
    r = _run_gate(json.dumps({"tool_input": {"x": 1}}), gate_return={"outcome": "allow"})
    assert r.exit_code == 2
...
def test_gate_receives_raw_tool_call():
    ...
    with patch("dina_cli.main.DinaClient") as MockCls, patch(
        "dina_cli.main.load_config", return_value=_cfg()
    ), patch("signal.signal"), patch("signal.alarm"):
        inst = MockCls.return_value
        inst.gate.return_value = {"outcome": "allow"}
        CliRunner().invoke(
            cli, ["gate-hook"],
            input=json.dumps({"tool_name": "Write", "tool_input": {"file_path": "x.ts"},
                              "cwd": "/work", "session_id": "claude-abc"}),
        )
...
pytestmark_posix = pytest.mark.skipif(os.name != "posix", reason="supervisor is a POSIX sh script")

...
1. If any command returns `pending_approval`, you MUST NOT proceed with that action — not "for demonstration", not ever — until its status command returns `approved`. Tell the user it's waiting on their phone, work on something else, re-check later. Trust the status, not the user's word.
2. Never bypass a denied or pending approval. Denied means no.

**Core commands:** `dina session start|end|list`, `dina ask <query> --session <id>`, `dina remember <text> --session <id>`, `dina validate <action> <description> --session <id> --context '<json>'`, `dina scrub` / `dina rehydrate`, `dina status`. For machine-readable output, put the global flag before the command: `dina --json <command> ...`.

...
| ID | Arch Source | Requirement | Recommended Section | Status |
|----|------------|-------------|---------------------|--------|
| M1 | `08-layer3-trust-network.md` §PeerLens AppView | Brain queries PeerLens AppView API (`GET /v1/trust?did=...`) to get product scores, expert attestations, and bot scores for recommendations. Phase 2 feature but AppView query contract should be tested. | §6 MCP Client (new subsection) | CLOSED |
| M2 | `08-layer3-trust-network.md` §Cold Start | When PeerLens AppView is unavailable, brain degrades gracefully to web search via OpenClaw. No disruption to user. Phase 1 behavior (AppView doesn't exist yet, brain uses web search). | §6 MCP Client | CLOSED |
| M3 | `10-layer5-bot-interface.md` §Bot Trust Scoring | Brain maintains per-bot PeerLens ratings locally. After each interaction outcome, brain recalculates bot score. Next query routes to updated best bot. TST-BRAIN-228 says "highest score selected" but doesn't verify local tracking or recalculation. | §6.1 Agent Routing | CLOSED |

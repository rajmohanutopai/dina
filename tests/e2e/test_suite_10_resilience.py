...
# TST-E2E-047
    def test_brain_crash_scratchpad_resume(
        ...
        brain, confirm scratchpad retains step 2 results, then complete
        remaining steps.
        """
        ...
        # Simulate completing steps 1 and 2
        step_results = {
        ...
        # Advance clock past the task timeout so watchdog triggers
        node.set_test_clock(time.time() + 15)

        # Watchdog check should reset the timed-out task
        ...
        # Scratchpad survives the crash -- step 2 results are intact
        ...
        # Resume from step 3 and complete remaining steps
        recovered["step_3"] = {"status": "done", "data": "ranked_sellers"}
        recovered["step_4"] = {"status": "done", "data": "draft_comparison"}
        recovered["step_5"] = {"status": "done", "data": "final_verdict"}
        recovered["completed_steps"] = 5
        node.write_scratchpad(task_id, recovered)

        # Mark task as completed
        node.tasks[task_id].status = TaskStatus.COMPLETED

        final = node.read_scratchpad(task_id)
        assert final["completed_steps"] == 5
        assert node.tasks[task_id].status == TaskStatus.COMPLETED
...
# TST-E2E-048
    def test_core_wal_recovery_after_power_loss(
        ...
        verify committed items are present and integrity check passes.
        """
        node = fresh_don_alonso
        node.first_run_setup("alonso@example.com", "passphrase123")
        node.create_persona("consumer", PersonaType.CONSUMER, "open")

        # Rapidly store 100 items
        stored_ids = []
        for i in range(100):
        ...
        # Record which items are committed (snapshot before crash)
        committed_ids = set(persona.items.keys())
        assert len(committed_ids) >= 100

        # Simulate power loss: remove some items (as if WAL replay lost them)
        items_to_remove = list(persona.items.keys())[:10]
        for item_id in items_to_remove:
            del persona.items[item_id]

        # After crash, 90 items remain
        assert len(persona.items) == 90

        # Verify the remaining 90 committed items are intact
        for item_id in persona.items:
            item = persona.items[item_id]
            assert item.persona == "consumer"
            assert item.body_text != ""
            assert item.item_id == item_id

        # Integrity check: every remaining item is self-consistent and readable.
        # After a simulated power loss the FTS index may be stale (entries
        # removed without index cleanup), so we verify directly against
        # the authoritative persona store -- the WAL recovery equivalent.
        for item_id, item in persona.items.items():
            assert item.item_id == item_id
            assert item.persona == "consumer"
            assert "data_block" in item.body_text

        # Items that survived the crash are self-consistent

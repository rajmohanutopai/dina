// Package internal implements the Dina D2D relay/mailbox.
//
// The relay is a lightweight encrypted mailbox. Home nodes connect via
// outbound WebSocket, authenticate with Ed25519, and receive messages
// pushed by other nodes. The relay never decrypts — it forwards NaCl
...
// RelayConn wraps a WebSocket connection with its owning DID.
...
func (h *Hub) Register(conn *RelayConn) {
	h.mu.Lock()
...
func (h *Hub) Unregister(did string, conn *RelayConn) {
	h.mu.Lock()
	defer h.mu.Unlock()

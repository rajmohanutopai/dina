...
// AuthChallenge is sent by the relay on new WebSocket connections.
...
// AuthTimeout is how long the relay waits for a valid auth response.
...
func Authenticate(ctx context.Context, ws *websocket.Conn) (string, error) {
	...
	// The relay accepts any DID format (did:plc, did:key, etc.) as long as
	// the signature verifies with the provided public key. The relay is a

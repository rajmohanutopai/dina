...
// PeerlensService orchestrates the local trust cache, AppView resolver,
// and contact directory for ingress gatekeeper decisions.
type PeerlensService struct {
	cache    port.PeerlensCache
	resolver port.PeerlensResolver
	contacts port.ContactLookup
}

// NewPeerlensService creates a trust service.
func NewPeerlensService(cache port.PeerlensCache, resolver port.PeerlensResolver, contacts port.ContactLookup) *PeerlensService {
	return &PeerlensService{
		cache:    cache,
		resolver: resolver,
		contacts: contacts,
	}
...
func (s *PeerlensService) EvaluateIngress(senderDID string) domain.IngressDecision {
	if senderDID == "" {
...
func (s *PeerlensService) SyncNeighborhood(ownDID string) error {
	if ownDID == "" {
...
func (s *PeerlensService) ResolveProfile(did string) (json.RawMessage, error) {
	return s.resolver.ResolveFullProfile(did)
}

// SearchPeerlens queries AppView for product/entity trust attestations.
func (s *PeerlensService) SearchPeerlens(query, category, subjectType string, limit int) (json.RawMessage, error) {
	return s.resolver.SearchPeerlens(query, category, subjectType, limit)
}

// GetCacheEntries returns all entries in the trust cache (for admin UI).
func (s *PeerlensService) GetCacheEntries() ([]domain.TrustEntry, error) {
	return s.cache.List()
}

// GetCacheStats returns cache summary statistics (for admin UI).
func (s *PeerlensService) GetCacheStats() (domain.TrustCacheStats, error) {
	return s.cache.Stats()
}

// ManualSync triggers an immediate sync (admin "Sync Now" button).
func (s *PeerlensService) ManualSync(ownDID string) (int, error) {
	entries, err := s.resolver.ResolveNeighborhood(ownDID, 2, 500)
	if err != nil {
		return 0, err
	}

...
// PeerlensCache provides read/write access to the local trust neighborhood cache.
// Implementations must be safe for concurrent use.
type PeerlensCache interface {
...
// PeerlensResolver fetches PeerLens profiles from an external source (AppView).
type PeerlensResolver interface {
	...
	SearchPeerlens(query, category, subjectType string, limit int) (json.RawMessage, error)
}

...
export default function SubjectDetailScreen(
  ...
  const {
    ...
    onWriteReview = (id: string) => {
      ...
      router.push({ pathname: '/peerlens/write', params: writeParams });
    },
    onPressOwnReview = () => {
      ...
      {detail.alternatives.length > 0 && (
        <AlternativesStrip
          ...
          onSelectAlternative={(altId) =>
            router.push({ pathname: '/peerlens/[subjectId]', params: { subjectId: altId } })
          }
        />
      )}
    </ScrollView>
...
function ReviewRow(props: ReviewRowProps): React.ReactElement {
  ...
  const band: PeerlensBand = trustBandFor(review.reviewerTrustScore);

 ...
 * the PeerLens tab, every drill-down (`/peerlens/search`, `/peerlens/[subjectId]`,
 * `/peerlens/reviewer/[did]`, …) became a global tab transition under
...
function PeerlensIndexHeader(): React.ReactElement {
 ...
 * a deep route (e.g. `/peerlens/search`) WITHOUT pushing the index
 ...
 */
function PeerlensStackBack({ canGoBack }: { canGoBack: boolean }): React.ReactElement {
  const router = useRouter();
  const onPress = () => {
    if (canGoBack) {
...
export default function PeerlensStackLayout(): React.ReactElement {
  return (
    <Stack
      screenOptions={{
        headerStyle: {
        ...
        // `PeerlensStackBack` above for why the auto-chevron isn't
        // sufficient on rehydrated stacks. `canGoBack` is the
        // Stack's own value, not the parent navigator's.
        headerLeft: ({ canGoBack }) => (
          <PeerlensStackBack canGoBack={canGoBack ?? false} />
        ),
      }}
    >
      <Stack.Screen

...
export default function WriteScreen(props: WriteScreenProps = {}): React.ReactElement {
  ...
  // fresh-compose resets that wipe the form below — so it's STABLE across a
  // re-submit of the SAME compose (load-bearing: it's the dedup/supersede key, so
  // a corrected re-submit after a failure replaces the stale job rather than
  // publishing a duplicate / overwriting the prior public record) yet FRESH when
  // the user starts a different compose without remounting (new params, or a
  // refocus). Edit-mode regen is idempotent (the maker returns the same rkey).
  const makePublishRkey = React.useCallback((): string => {
    const editRkey = paramEditingUri?.split('/').pop();
    return editRkey !== undefined && editRkey.length > 0
      ? editRkey
      : `mob-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
  }, [paramEditingUri]);
  const publishRkeyRef = React.useRef<string | null>(null);
  if (publishRkeyRef.current === null) publishRkeyRef.current = makePublishRkey();
  const {
    ...
    onPublish = async (formState: WriteFormState) => {
      ...
      // `publishRkeyRef` is stable per compose session (regenerated only by the
      // fresh-compose resets below; edit mode reuses the original record's rkey)
      // so a corrected re-submit supersedes the prior job instead of duplicating
      // it, while a brand-new compose gets a fresh key.
      const publishRkey = publishRkeyRef.current ?? makePublishRkey();
      ...
      try {
        ...
        // If we came from a chat draft, persist the FORM-edited values onto that
        // draft's lifecycle for EVERY outcome — published/queued (then navigate),
        // but also a permanent failure (`error`, which created a durable FAILED
        // job) or a rejection. Whatever the inline card later projects — a
        // cancelled queue, a dismissed failure, or a re-opened ReadyState — must
        // show THESE edits, not the stale pre-form (LLM) values.
        if (
          paramDraftId !== undefined &&
          paramDraftId.length > 0 &&
          paramThreadId !== undefined &&
          ...
          setReviewDraftStatus(paramThreadId, paramDraftId, 'ready', { values: formState });
        }
        ...
        // submitReviewFromUI can THROW (not just return an outcome) — e.g. a
        // SQLite transaction/insert error from the durable-job write. Surface it
        // inline (the inline card already catches the same path); without this the
        // spinner clears via `finally` with no error and the promise rejects
        // unhandled (handlePublish doesn't await/catch this default).
        setLocalError(e instanceof Error ? e.message : 'Publish failed.');
      } finally {
  ...
  React.useEffect(() => {
    ...
    publishRkeyRef.current = makePublishRkey(); // fresh compose → fresh rkey (edit-mode idempotent)
    setLocalError(null);
    setShowErrors(false);
  }, [defaultInitial, props.initial, makePublishRkey]);
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      setState(defaultInitial);
      publishRkeyRef.current = makePublishRkey(); // refocus = fresh compose → fresh rkey
      setLocalError(null);
      setShowErrors(false);
    }, [defaultInitial, props.initial, editing, makePublishRkey]),
  );

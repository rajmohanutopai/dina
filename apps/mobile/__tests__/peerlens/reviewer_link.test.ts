 ...
 *     `/peerlens/reviewer/<did>[?namespace=<frag>]` per plan §8.5
 *     and §3.5.
 ...
 *     hard-null on non-/peerlens/reviewer/ paths or bad DIDs.
...
} from '../../src/peerlens/reviewer_link';
...
describe('buildReviewerProfileDeepLink — plain (no namespace)', () => {
  it('returns /peerlens/reviewer/<did> for a valid did:plc', () => {
    expect(buildReviewerProfileDeepLink({ did: VALID_DID })).toBe(
      `/peerlens/reviewer/${encodeURIComponent(VALID_DID)}`,
    );
  });
  ...
  it('handles did:web (encoded for path-segment safety)', () => {
    ...
    expect(out).toBe('/peerlens/reviewer/did%3Aweb%3Aexample.com');
  });
...
describe('buildReviewerProfileDeepLink — namespaced', () => {
  it('appends ?namespace=<frag> for a valid namespace', () => {
    ...
    expect(out).toBe(
      `/peerlens/reviewer/${encodeURIComponent(VALID_DID)}?namespace=namespace_3`,
    );
  });
...
describe('parseReviewerProfileDeepLink — happy paths', () => {
  it('parses a plain /peerlens/reviewer/<did>', () => {
    const url = `/peerlens/reviewer/${encodeURIComponent(VALID_DID)}`;
  ...
  it('parses /peerlens/reviewer/<did>?namespace=<frag>', () => {
  ...
  it('first namespace wins (RFC-3986-undefined; explicit pin)', () => {
    const url = `/peerlens/reviewer/${encodeURIComponent(VALID_DID)}?namespace=ns_a&namespace=ns_b`;
...
describe('parseReviewerProfileDeepLink — graceful degradation', () => {
  it('namespace round-trips through encodeURIComponent (unrestricted charset)', () => {
    ...
    const url = `/peerlens/reviewer/${encodeURIComponent(VALID_DID)}?namespace=has%20space`;
  ...
  it('namespace exceeding 255 chars → namespace: null', () => {
    ...
    const url = `/peerlens/reviewer/${encodeURIComponent(VALID_DID)}?namespace=${huge}`;
    expect(parseReviewerProfileDeepLink(url)?.namespace).toBeNull();
  });

  it('returns null for non-/peerlens/reviewer paths', () => {
    expect(parseReviewerProfileDeepLink('/peerlens/subject-aeron')).toBeNull();
    expect(parseReviewerProfileDeepLink('https://example.com')).toBeNull();
    expect(parseReviewerProfileDeepLink('/peerlens/reviewer')).toBeNull(); // missing trailing slash + did
  });

  it('returns null for empty did segment', () => {
    expect(parseReviewerProfileDeepLink('/peerlens/reviewer/')).toBeNull();
  });

  it('returns null for invalid DIDs in the path (defensive)', () => {
    expect(parseReviewerProfileDeepLink('/peerlens/reviewer/banana12')).toBeNull();
    // Length below 8-char min length cap.
    expect(parseReviewerProfileDeepLink('/peerlens/reviewer/did:p:x')).toBeNull();
  });
  ...
  it('exact-match query key — `namespaceFoo=...` does NOT match `namespace=`', () => {
    const url = `/peerlens/reviewer/${encodeURIComponent(VALID_DID)}?namespaceFoo=ns_1`;
    expect(parseReviewerProfileDeepLink(url)?.namespace).toBeNull();
  });
});

 ...
 *     else.
 *   - Deep link: `/peerlens/<subjectId>?attestation=<at-uri>` per plan
...
describe('buildCosigInboxRow — deep link', () => {
  it('routes to /peerlens/<subjectId>?attestation=<at-uri> with the source attestation anchored', () => {
  ...
  it('encodes special characters in the subject id (path segment encoding preserved)', () => {
    ...
    expect(r.deepLink).toMatch(/^\/peerlens\/subj%20a%2Fb\?attestation=/);
  });

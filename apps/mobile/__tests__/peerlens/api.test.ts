...
import {
  ...
  resetSearchCache,
} from '../../../../packages/core/src/peerlens/network_search';
import {
  PeerlensQueryClient,
  type PeerlensProfile,
} from '../../../../packages/core/src/peerlens/query_client';
import {
  subscribeTrust,
...
function profile(did: string, score: number | null = 0.5): PeerlensProfile {
  return {
    did,
...
class FakeQueryClient extends PeerlensQueryClient {
  public profileCalls = 0;
  public searchCalls = 0;
  private profileHandler: (did: string) => PeerlensProfile | null;

  constructor(profileHandler: (did: string) => PeerlensProfile | null) {
...
describe('subscribeTrustSearch', () => {
  it('canonicalises queries — different property orders share one subscriber set', async () => {
    ...
    const q1: PeerlensSearchQuery = { type: 'entity_reviews', query: 'acme', limit: 5 };
    const q2: PeerlensSearchQuery = { limit: 5, query: 'acme', type: 'entity_reviews' };

    const a: (PeerlensSearchResult | null)[] = [];
    const b: (PeerlensSearchResult | null)[] = [];
  ...
  it('initial fire delivers the search result to the listener', async () => {
    ...
    const seen: (PeerlensSearchResult | null)[] = [];
    subscribeTrustSearch(
...
describe('invalidateTrustSearch', () => {
  it('drops the underlying cache row, refetches, and notifies subscribers with the fresh result', async () => {
    ...
    const seen: (PeerlensSearchResult | null)[] = [];
    const q: PeerlensSearchQuery = { type: 'identity_attestations', query: 'did:plc:zzz' };
...
describe('invalidateAll', () => {
  it('re-runs both PeerLens rating and search subscriptions', async () => {
    ...
    const searchSeen: (PeerlensSearchResult | null)[] = [];

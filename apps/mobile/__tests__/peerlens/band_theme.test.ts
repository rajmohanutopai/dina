 ...
 * (`src/peerlens/band_theme.ts`).
 *
 * The `BAND_COLOUR` + `BAND_LABEL` maps are imported by three
 * surfaces — `subject_card_view.tsx`, `app/peerlens/[subjectId].tsx`,
 * `app/peerlens/reviewer/[did].tsx` — so structural drift here breaks
 * the score-badge / mini-band rendering everywhere at once. These
 * tests guard:
 *
 *   1. **Total coverage** — every `PeerlensBand` union value has a non-
...
import { trustScoreLabel, type PeerlensBand } from '../../src/peerlens/score_helpers';

const ALL_BANDS: readonly PeerlensBand[] = [
  'high',
  'moderate',
  'low',
  'very-low',
...
describe('band_theme — total coverage', () => {
  ...
  it('BAND_COLOUR has exactly the PeerlensBand keys (no extras, no gaps)', () => {
    expect(Object.keys(BAND_COLOUR).sort()).toEqual([...ALL_BANDS].sort());
  });

  it('BAND_LABEL has exactly the PeerlensBand keys (no extras, no gaps)', () => {
...
describe('band_theme — frozen invariants', () => {
  ...
  it('mutating BAND_COLOUR throws (strict mode)', () => {
    expect(() => {
      (BAND_COLOUR as Record<PeerlensBand, string>).high = '#000000';
    }).toThrow(TypeError);
  });

  it('mutating BAND_LABEL throws (strict mode)', () => {
    expect(() => {
      (BAND_LABEL as Record<PeerlensBand, string>).high = 'TAMPERED';
    }).toThrow(TypeError);
  });
...
describe('band_theme — label content', () => {
  ...
  it('rated band labels are non-empty uppercase strings (regression guard)', () => {
    const ratedBands: readonly PeerlensBand[] = ['high', 'moderate', 'low', 'very-low'];

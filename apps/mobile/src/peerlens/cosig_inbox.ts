 ...
 * Per plan §10:
 *
 *   > An inbound `peerlens.cosig.request` lands in the unified inbox
 ...
 *   > the source attestation in `app/peerlens/[subjectId]` with an
 ...
 *   - **Deep-link target** — `/peerlens/<subjectId>` per the plan; the
...
export interface CosigInboxRowDisplay {
  ...
  /**
   * Where a tap should route. `app/peerlens/[subjectId]?attestation=...`

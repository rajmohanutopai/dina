 ...
 *   - The deep-link pattern `/peerlens/<subjectId>` is also from
...
export interface CosigNotificationBody {
   ...
   * `/peerlens/<encodeURIComponent(subjectId)>` — same pattern as
...
function buildDeepLink(subjectId: string): string {
  ...
  return `/peerlens/${encodeURIComponent(subjectId)}`;
}

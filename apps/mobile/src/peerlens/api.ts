 ...
 *   - The wire-format types (`PeerlensProfile`, `PeerlensReview`, etc.) —
 *     re-exported from `@dina/core` via this file's surface.
 *   - The actual cache + KV machinery — `@dina/core/trust/cache`.
 *   - The xRPC client (`PeerlensQueryClient`) — `@dina/core/trust/query_client`.
...
export type {
  ...
  PeerlensProfile,
  PeerlensSearchQuery,
  PeerlensSearchResult,
  PeerlensSearchFilters,
  PeerlensReview,
} from '@dina/core';

type TrustListener = (score: TrustScore | null) => void;
type SearchListener = (result: PeerlensSearchResult | null) => void;
...
/**
 * Build the canonical cache key for a `PeerlensSearchQuery`. Identical
 ...
 */
export async function invalidateTrustSearch(query: PeerlensSearchQuery): Promise<void> {
  const key = searchKey(query);
...
function notifySearch(key: string, result: PeerlensSearchResult | null): void {
  const entry = searchSubscribers.get(key);
  if (!entry) return;
  for (const l of [...entry.listeners]) l(result);
}

async function safeSearch(query: PeerlensSearchQuery): Promise<PeerlensSearchResult | null> {
  try {

...
import type { PeerlensBand } from '@dina/protocol';
...
export interface SubjectReview {
   ...
   * `/peerlens/write` in edit mode (the screen reads `editingUri` to
   ...
   * pre-seed the body field in `/peerlens/write`, matching the
   * reviewer-profile Edit affordance.
   */
  readonly body?: string;
  /**
   * Review sentiment. Carried so the Edit pill can pre-select the
   * correct sentiment chip in `/peerlens/write` instead of defaulting

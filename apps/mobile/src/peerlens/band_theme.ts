 ...
 * `PeerlensBand` values to (a) a colour token from the theme palette and
 * (b) a human-readable label string. Until iter-53 each call site
 * re-declared its own `BAND_COLOUR: Record<PeerlensBand, string>` and
 * `BAND_LABEL: Record<PeerlensBand, string>` map — four copies of the
 ...
 */
export const BAND_COLOUR: Readonly<Record<PeerlensBand, string>> = Object.freeze({
  high: colors.success,
 ...
 */
export const BAND_LABEL: Readonly<Record<PeerlensBand, string>> = Object.freeze({
  high: 'HIGH',
  moderate: 'MODERATE',
  low: 'LOW',
  'very-low': 'VERY LOW',

 ...
 * Plan §10:
 *
 *   > An inbox tap should land on `app/peerlens/[subjectId]` AND surface
 *   > the source attestation that triggered the row — scrolled to,
 *   > highlighted, with the action sheet pinned over it.
 *
 * Today the cosig inbox row's `deepLink` is just `/peerlens/<subjectId>`
...
export interface AttestationDeepLinkInput {
   ...
   * plain `/peerlens/<subjectId>` form (no anchor). The screen falls
...
const PEERLENS_BASE_PATH = '/peerlens/';
...
export function buildAttestationDeepLink(input: AttestationDeepLinkInput): string {
  ...
  const path = PEERLENS_BASE_PATH + encodeURIComponent(input.subjectId);
 ...
 *
 * Returns `null` for non-`/peerlens/...` URLs and other non-deep-link
...
export function parseAttestationDeepLink(url: unknown): ParsedAttestationDeepLink | null {
  ...
  // Only handle the path shape we own — `/peerlens/<subjectId>[?...]`.
  if (!url.startsWith(PEERLENS_BASE_PATH)) return null;
  ...
  const subjectIdEncoded = pathPart.slice(PEERLENS_BASE_PATH.length);

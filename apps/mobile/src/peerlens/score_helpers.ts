...
/**
 * Human-friendly band label for a `PeerlensBand`.
 *
 ...
 */
export function bandDisplayName(band: PeerlensBand): string {
  switch (band) {
    case 'high':
      return 'High trust';
    case 'moderate':
...
export function bandColorToken(band: PeerlensBand): TrustBandToken {
  switch (band) {
    case 'high':
      return 'success';
    case 'moderate':

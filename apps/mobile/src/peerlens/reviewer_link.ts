 ...
 *   > `app/peerlens/reviewer/[did]` with that reviewer's profile.
 *
 * Today only the screen route exists (TN-MOB-015). Every surface that
 * shows a reviewer needs to compose the same `/peerlens/reviewer/<did>`
 ...
 * (`/peerlens/reviewer/<did>` vs `/peerlens/<subjectId>`). Same builder
...
const REVIEWER_BASE_PATH = '/peerlens/reviewer/';
 ...
 * `/peerlens/reviewer/...` paths or malformed input. Like the inbox

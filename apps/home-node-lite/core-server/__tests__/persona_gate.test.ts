...
describe('checkPersonaGate — input validation', () => {
  it.each([
    ...
    ['bogus tier', { ...input(), persona: { name: 'x', tier: 'bogus' as PersonaGateTier, open: true } }],
    ['non-boolean open', { ...input(), persona: { name: 'x', tier: 'default', open: 1 as unknown as boolean } }],
...
describe('checkPersonaGate — default tier', () => {
  it.each(['user', 'brain', 'admin'] as const)(
    '%s reading default tier → allowed',
    (role) => {

/**
 * Task 4.60 — PLC update operation.
 *
 ...
 * (resolve + parse the current doc via `@dina/core.resolveDIDPLC`).
 * Separation keeps the update builder pure + testable.
 *
 ...
 * `@dina/core` — same canonical dag-cbor + SHA-256 + ECDSA low-s
 ...
 *
 * Source: docs/HOME_NODE_LITE_TASKS.md Phase 4g task 4.60.
 */
...
import {
  ...
  signOperation,
  type PLCDirectoryConfig,
} from '@dina/core';
import { base58 } from '@scure/base';

/** Default PLC directory — mirrors `DEFAULT_PLC_DIRECTORY` in `@dina/core/constants`. */
 ...
 * Returns the op ready for `signOperation` from `@dina/core`.
 */
 ...
 * ceremony the admin CLI uses. Returns the signed op + the DID
 * + the operation hash. When `config.fetch` is wired, also POSTs
 * to the PLC directory and throws on non-2xx.
 */

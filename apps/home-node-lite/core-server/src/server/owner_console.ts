...
(function () {
  ...
  function loadSetup() {
    api("GET", "/v1/owner/setup/status").then(function (data) {
      ...
      renderCodingAgents(Array.isArray(data.coding_agents) ? data.coding_agents : []);
    }).catch(function (e) {
  ...
  function renderCodingAgents(agents) {
    ...
    agents.forEach(function (agent) {
      ...
      row.appendChild(el("span", { text: String(agent.name || "Coding agent") }));
      row.appendChild(el("code", { text: String(agent.did || "") }));
  ...
  document.getElementById("save").addEventListener("click", function () {
    ...
    refreshKeyState();
    loadSetup(); loadRuns(); loadWatches();
  });
  ...
  refreshKeyState();
  if (getCap()) { loadSetup(); loadRuns(); loadWatches(); }
})();
</script>
</body>
</html>`;

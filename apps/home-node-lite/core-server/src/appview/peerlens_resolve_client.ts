...
const VALID_SEVERITIES: ReadonlySet<PeerlensFlag['severity']> = new Set([
  'critical',
  'serious',
  'warning',
  'info',
...
function parseFlags(v: unknown): PeerlensFlag[] {
  if (!Array.isArray(v)) return [];
  const out: PeerlensFlag[] = [];
  for (const entry of v) {
    if (entry === null || typeof entry !== 'object') continue;
    const e = entry as Record<string, unknown>;
    if (typeof e.flagType !== 'string' || e.flagType === '') continue;
    const severity = e.severity as PeerlensFlag['severity'];
...
function parseGraph(v: unknown): PeerlensGraphContext | null {
  if (v === null || typeof v !== 'object' || Array.isArray(v)) return null;

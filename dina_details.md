...
-- Our primary test surface is home node lite web - simulator testing is last mile only - just before release
...
## How structured service results are enforced (eta_query etc.)
The agent is never trusted to produce the structure — it is declared, routed, instructed, frozen, and validated:
1. The listing declares `capabilitySchemas.<cap> = {params, result, schemaHash}`; the requester pins `schema_hash` at query time (mismatch → `schema_version_mismatch`).
2. Provider ingress validates incoming params against the published schema BEFORE minting any task (`service_handler.validateParams`).
3. The capability's `mcpServer` rides the workflow task as `requested_runner`; a daemon's `runner_filter` only claims matching tasks — so structured service tasks can never be claimed by free-text runners (claude-code/codex/gemini/openclaw-cli serve only unconstrained user tasks). Two lanes by construction.
4. The task prompt's `service_query_execution` branch embeds the exact params JSON and demands the tool's JSON output VERBATIM (compact, no prose) — a summary is a schema violation.
5. Backstop: at task creation the provider freezes a `schemaSnapshot` (GAP-SH-03); `response_bridge_sender` validates the runner's success result against that frozen snapshot before sending (GAP-SH-05) — non-conformant output reaches the requester as a clean `result_schema_violation` error, never as data. The requester renders the typed result into the card (eta_minutes → "3 min to Castro Street").
Known gap: the headless CLI runners' envelope has no structured branch — if a listing ever declares `mcpServer: claude-code`, the task would claim but fail validation (requester still protected). Extending the headless envelope (params JSON + JSON-only stdout + extraction) is the follow-up if free-text agents should serve structured capabilities.

## Provider tiers — how someone offers a service (full design: docs/SERVICE_PROVIDER_TIERS.md)
Three tiers by WHERE the answer lives. Tier 1 (prompt provider): the answer lives in the provider's head/notes — they publish a listing + write "How should Dina answer?" in plain language; their OWN Dina executes queries in-process (LocalDelegationRunner + brain runCapability with vault search + schema-constrained output); they maintain the service by /remember ("mangoes out till Thursday"). No code, no daemon — a phone and a sentence. Tier 2 (agent provider): answer lives in their agent's tools — dina init + agent-daemon with a headless runner; needs the structured task envelope (gap). Tier 3 (developer): external systems — custom runner/MCP (the bus42 pattern). Structure is safe at every tier: the Response Bridge validates against the frozen schemaSnapshot regardless of who answered.
Key insights: Tier 1's competitor is "call the guy and wait", not an API; ~70% of realistic services are Tier 1 (quotes, prices, hours, status-by-human, homework, menus, timings); service_quote + order_status are the killer Tier 1 capabilities; known_only+grants pairs naturally with Tier 1 (subject-scoped = human-scale); responsePolicy review is the transactional escape hatch (bookings via approval card); tiers attach PER CAPABILITY (requested_runner routing already supports it). Tier 1 prerequisite: fix the MsgBox WS idle-staleness (no auto-reconnect) — the phone IS the service.

## Tier 1 prompt-provider — SHIPPED + live-proven (2026-06-11, salon demo)

...
| 1.10a | Gatekeeper: Brain→Core API auth | Infra | **Brain is an untrusted tenant.** Shared secret between brain and core, rotated on each boot (Docker Secrets). Per-endpoint authorization: brain can call `/v1/vault/query` but background tasks cannot call `/v1/did/sign`. Persona access tiers enforced in `gatekeeper.go`: open (serve + log), restricted (serve + log + notify), locked (reject until user unlocks with TTL). Every brain API call requires secret in `Authorization` header. | 1.7, 1.2 | dina-core + brain | NOT STARTED |
...
| # | Item | Layer | What It Is | Depends On | Container | Status |
|---|------|-------|-----------|-----------|-----------|--------|
| 1.11 | Gmail connector (read-only) | L2 Ingestion | OAuth read-only access with token lifecycle state machine (ACTIVE → NEEDS_REFRESH → EXPIRED → REVOKED). Auto-refresh before expiry. Tier 2 notification on failure. **Two-pass Ingestion Triage:** Pass 1 — Gmail category filter (PROMOTIONS/SOCIAL/UPDATES/FORUMS → thin record). Pass 2 — within PRIMARY, regex pre-filter + LLM batch classification of subject+sender (50 per call, ~$0.003/year) to separate real correspondence from newsletters/notifications/recruiter spam. Only emails classified as INGEST get full download. Fiduciary override: security alerts, financial docs, expirations always ingested. Attachments: metadata only (filename, size, MIME type), full content stays in source — never store binary blobs in SQLite. **Living Window sync:** fast sync (last 30 days) → status READY → Historian backfills to `DINA_HISTORY_DAYS` (default 365) in background batches of 100. Historian pauses on user query. Data older than horizon: pass-through API search only, never saved to vault. | 1.7, 1.10 | dina-core + brain (triage LLM) | NOT STARTED |
...
| 1.29a | Container image signing | Infra | Sign container images with cosign (Sigstore). Pin base image digests (not tags) in Dockerfile. Ship SBOM with each release. Reproducible builds where possible. Prevents supply chain attacks via compromised base images. | 1.10 | CI/CD | NOT STARTED |
| 1.29b | Monitoring & self-notifications | Infra | dina-core exposes `/metrics` (Prometheus format). Key metrics: vault size per persona, connector last-sync time, LLM latency p50/p95, message queue depth, disk usage. Critical metrics trigger Tier 2 self-notifications: "Gmail hasn't synced in 48 hours", "Disk 90% full", "Brain container restarted 3 times in 1 hour". | 1.7, 1.16 | dina-core | NOT STARTED |
...
| 1.36 | MCP integration (OpenClaw) | L7 Action | Delegate tasks to OpenClaw via MCP. License renewal, form filling, task automation — all with `draft_only` constraint. | 1.25 | dina-brain | NOT STARTED |
...
| 2.16 | Confidential Computing (pilot) | Infra | AWS Nitro Enclaves / AMD SEV-SNP for managed hosting. Remote attestation proves unmodified binary. Eliminates honeypot problem at hardware level. **Also:** for VPS convenience mode, encrypt DEK with vTPM-backed key where available (AWS/GCP/Azure) — prevents offline attacks from stolen disk images. | 1.33 | NOT STARTED |
...
| 2.16 vTPM for convenience mode | 2 | Issue #13 — root on VPS = full compromise |

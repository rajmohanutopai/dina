...
| 1.10a (updated) Three-tier auth | 1a | Issue #12 — `BRAIN_TOKEN` (boot-generated) + `CLIENT_TOKEN` (admin web UI login only) + `Ed25519 signatures` (all client devices). No JWTs. Static allowlist in `gatekeeper.go`. Brain never calls `/v1/did/sign` — triggers high-level ops, core handles crypto. Permanent design (no plugins). |
...
| (architectural decision) Kernel model | All | Dina has no plugins. Child agents (OpenClaw etc.) are external processes via MCP. NaCl over HTTPS for peers. Three-tier auth (BRAIN_TOKEN + CLIENT_TOKEN + Ed25519) is the permanent design. |

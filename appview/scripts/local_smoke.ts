...
async function main() {
  ...
  await attHandler.handleCreate(ctx, {
    uri: `at://${ALICE}/com.dinakernel.peerlens.attestation/3kabcd1`,
    did: ALICE,
  ...
  await attHandler.handleCreate(ctx, {
    uri: `at://${BOB}/com.dinakernel.peerlens.attestation/3kabcd2`,
    did: BOB,
    collection: 'com.dinakernel.peerlens.attestation',
    rkey: '3kabcd2',
    cid: 'bafyreiabc2',
  ...
  console.log('--- 6. hit xRPC com.dinakernel.peerlens.search ---')
  const search = await fetch(`${WEB_URL}/xrpc/com.dinakernel.peerlens.search?q=Aeron`).then(r => r.json())
  console.log(' search results:', JSON.stringify(search, null, 2))

  console.log('--- 7. hit xRPC com.dinakernel.peerlens.networkFeed (alice) ---')
  const feed = await fetch(`${WEB_URL}/xrpc/com.dinakernel.peerlens.networkFeed?viewerDid=${encodeURIComponent(ALICE)}`).then(r => r.json())
  console.log(' feed:', JSON.stringify(feed, null, 2))

  console.log('--- 8. hit xRPC com.dinakernel.peerlens.getAttestations (subject = shop) ---')
  const att = await fetch(`${WEB_URL}/xrpc/com.dinakernel.peerlens.getAttestations?subject=${encodeURIComponent(SHOP)}`).then(r => r.json())

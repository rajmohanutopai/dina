...
import { peerlensV1Params } from '@/db/schema/peerlens-v1-params.js'
import { TRUST_V1_PARAM_SEEDS } from '@/db/seeds/peerlens-v1-params.js'
 ...
 * via the cache TTL.
 *
 * **Snapshot semantics**: one read returns a frozen `PeerlensV1Params` with
 ...
 */
export interface PeerlensV1Params {
...
interface CachedSnapshot {
  readonly value: PeerlensV1Params
 ...
 */
function buildSeedSnapshot(): PeerlensV1Params {
 ...
 */
export async function readPeerlensV1Params(db: DrizzleDB): Promise<PeerlensV1Params> {
  const seedSnapshot = buildSeedSnapshot()
  const rows = await db
    .select({ key: peerlensV1Params.key, value: peerlensV1Params.value })
    .from(peerlensV1Params)
 ...
 */
export async function readCachedTrustV1Params(db: DrizzleDB): Promise<PeerlensV1Params> {
  const now = Date.now()
  if (cached !== null && now - cached.loadedAt < PARAMS_CACHE_TTL_MS) {
    return cached.value
  }
  try {
    const fresh = await readPeerlensV1Params(db)

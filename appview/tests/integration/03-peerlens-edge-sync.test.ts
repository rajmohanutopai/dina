...
import { addTrustEdge, removeTrustEdge } from '@/ingester/peerlens-edge-sync'
...
describe('§3.1 Trust Edge Creation + Removal', () => {
  ...
  it('IT-TE-006: vouch delete → trust edge removed', async () => {
    ...
    // Verify trust edge exists
    const edgesBefore = await db.select().from(schema.peerlensEdges).where(eq(schema.peerlensEdges.sourceUri, uri))
  ...
  it('IT-TE-010: multiple edge types from same author to same target', async () => {
    ...
    const edges = await db.select().from(schema.peerlensEdges).where(
      and(
        ...
        eq(schema.peerlensEdges.toDid, SUBJECT_DID),
      ),
    )

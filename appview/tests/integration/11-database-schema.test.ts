...
describe('11.1 Schema Correctness', () => {
  ...
  it('IT-DB-003: attestations -- primary key on uri', async () => {
    ...
    const testUri = `at://did:plc:test11/com.dinakernel.peerlens.attestation/db003-${Date.now()}`
  ...
  it('IT-DB-028: ingest_rejections -- accepts a valid row (TN-DB-005)', async () => {
    ...
    const testUri = `at://did:plc:db028/com.dinakernel.peerlens.attestation/db028-${Date.now()}`
    await db.execute(sql.raw(`
  ...
  it('IT-DB-029: ingest_rejections -- duplicate (at_uri, reason) NOT unique (TN-DB-005)', async () => {
    ...
    const testUri = `at://did:plc:db029/com.dinakernel.peerlens.attestation/db029-${Date.now()}`
    await db.execute(sql.raw(`
  ...
  it('IT-DB-036: attestations -- language column nullable (TN-DB-008)', async () => {
    ...
    const testUri = `at://did:plc:db036/com.dinakernel.peerlens.attestation/db036-${Date.now()}`
    await db.execute(sql.raw(`
    ...
    const testUri2 = `at://did:plc:db036/com.dinakernel.peerlens.attestation/db036b-${Date.now()}`
    await db.execute(sql.raw(`
      INSERT INTO attestations (uri, author_did, cid, subject_ref_raw, category, sentiment, record_created_at, indexed_at)
  ...
  it('IT-DB-038: cosig_requests -- accepts valid pending row (TN-DB-003)', async () => {
    ...
    const testUri = `at://did:plc:db038/com.dinakernel.peerlens.attestation/db038-${Date.now()}`
    await db.execute(sql.raw(`
  ...
  it('IT-DB-039: cosig_requests -- status CHECK rejects unknown enum (TN-DB-003)', async () => {
    ...
    // reason about.
    const testUri = `at://did:plc:db039/com.dinakernel.peerlens.attestation/db039-${Date.now()}`
    await expect(
      db.execute(sql.raw(`
        INSERT INTO cosig_requests (requester_did, recipient_did, attestation_uri, status, expires_at)
  ...
  it('IT-DB-040: cosig_requests -- unique (requester_did, attestation_uri, recipient_did) (TN-DB-003)', async () => {
    ...
    const testUri = `at://did:plc:db040/com.dinakernel.peerlens.attestation/db040-${Date.now()}`
    await db.execute(sql.raw(`
      INSERT INTO cosig_requests (requester_did, recipient_did, attestation_uri, status, expires_at)
  ...
  it('IT-DB-046: attestations.namespace -- nullable + round-trip (TN-DB-012)', async () => {
    ...
    const testUriRoot = `at://did:plc:db046/com.dinakernel.peerlens.attestation/db046-root-${Date.now()}`
    const testUriNs = `at://did:plc:db046/com.dinakernel.peerlens.attestation/db046-ns-${Date.now()}`
    await db.execute(sql.raw(`
      INSERT INTO attestations (uri, author_did, cid, subject_ref_raw, category, sentiment, record_created_at, indexed_at)
  ...
  it('IT-DB-047: endorsements.namespace -- nullable + round-trip (TN-DB-012)', async () => {
    ...
    const testUri = `at://did:plc:db047/com.dinakernel.peerlens.endorsement/db047-${Date.now()}`
    await db.execute(sql.raw(`

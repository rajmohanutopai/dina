 ...
 * via D2D messages (`peerlens.cosig.request` / `trust.cosig.accept` /
...
import { attestations, cosigRequests, peerlensEdges } from '@/db/schema/index'
...
describe('TN-TEST-041 §2: acceptance closes request with endorsement_uri', () => {
  it('accepted row carries endorsement_uri navigable to the published endorsement', async () => {
    ...
    const edge = await db
      .select()
      .from(peerlensEdges)
      .where(eq(peerlensEdges.sourceUri, ENDORSE_URI))

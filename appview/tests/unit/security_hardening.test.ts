 ...
 *      pin per-collection daily caps including `com.dinakernel.peerlens.flag`
...
describe('rate-limit isolation — bypass-resistance', () => {
  ...
  it('per-IP isolation: ip-A exhausting cap does not affect ip-B', () => {
    ...
    const limit = getMethodLimit('com.dinakernel.peerlens.search') // 60

    // ip-A burns the bucket
    for (let i = 0; i < limit; i++) {
      const r = checkPerMethodRateLimit(cache, '1.1.1.1', 'com.dinakernel.peerlens.search', NOW)
      expect(r.ok).toBe(true)
    }
    // ip-A's next request is denied
    expect(
      checkPerMethodRateLimit(cache, '1.1.1.1', 'com.dinakernel.peerlens.search', NOW).ok,
    ).toBe(false)

    // ip-B is still fresh (proves bucket isolation)
    expect(
      checkPerMethodRateLimit(cache, '2.2.2.2', 'com.dinakernel.peerlens.search', NOW).ok,
    ).toBe(true)
  })
  ...
  it('per-method isolation: search exhaustion does not affect subjectGet', () => {
    ...
    const cache = createRateLimitCache()
    const ip = '1.1.1.1'
    const searchLimit = getMethodLimit('com.dinakernel.peerlens.search') // 60

    for (let i = 0; i < searchLimit; i++) {
      checkPerMethodRateLimit(cache, ip, 'com.dinakernel.peerlens.search', NOW)
    }
    expect(
      checkPerMethodRateLimit(cache, ip, 'com.dinakernel.peerlens.search', NOW).ok,
    ).toBe(false)

    // subjectGet is a separate bucket — fresh.
    expect(
      checkPerMethodRateLimit(cache, ip, 'com.dinakernel.peerlens.subjectGet', NOW).ok,
    ).toBe(true)
  })
  ...
  it('UNKNOWN_IP_BUCKET clients share one bucket (intentional — degraded clients pay together)', () => {
    ...
    const cache = createRateLimitCache()
    const limit = getMethodLimit('com.dinakernel.peerlens.search')

    // First client (call them A) exhausts the unknown bucket
    for (let i = 0; i < limit; i++) {
      checkPerMethodRateLimit(cache, UNKNOWN_IP_BUCKET, 'com.dinakernel.peerlens.search', NOW)
    }
    // A is now denied
    expect(
      checkPerMethodRateLimit(cache, UNKNOWN_IP_BUCKET, 'com.dinakernel.peerlens.search', NOW).ok,
    ).toBe(false)
  ...
  it('window roll-over: bucket resets at +60s', () => {
    ...
    expect(
      checkPerMethodRateLimit(cache, ip, 'com.dinakernel.peerlens.search', NOW + 60_001).ok,
    ).toBe(true)
  })
  ...
  it('env override CANNOT lower the static tier (HIGH-01 invariant)', () => {
    ...
    expect(getMethodLimit('com.dinakernel.peerlens.attestationStatus', 10)).toBe(600)
    expect(getMethodLimit('com.dinakernel.peerlens.search', 10)).toBe(60)
  })
})
...
describe('PER_METHOD_LIMITS_RPM — tier integrity', () => {
  ...
  it('outbox-polling tier (attestationStatus) is the highest', () => {
    ...
    expect(PER_METHOD_LIMITS_RPM['com.dinakernel.peerlens.attestationStatus']).toBe(
      Math.max(...allCaps),
    )
  })
})
...
describe('flag-spam (TN-TEST-082 — cross-cutting reference)', () => {
  ...
  it('com.dinakernel.peerlens.flag cap is 10/day (Plan §6.4 — strictest tier)', () => {
  ...
  it('cap tiers are ordered by abuse risk: flag(10) < endorsement(30) < attestation(60)', () => {
    ...
    const flag = getCollectionDailyCap('com.dinakernel.peerlens.flag')!
    const endorsement = getCollectionDailyCap('com.dinakernel.peerlens.endorsement')!
    const attestation = getCollectionDailyCap('com.dinakernel.peerlens.attestation')!
  ...
  it('un-capped collections (vouch, reaction, reply, reportRecord) return null', () => {
    ...
    expect(getCollectionDailyCap('com.dinakernel.peerlens.reaction')).toBeNull()
    expect(getCollectionDailyCap('com.dinakernel.peerlens.reply')).toBeNull()
    expect(getCollectionDailyCap('com.dinakernel.peerlens.reportRecord')).toBeNull()
  })
  ...
  it('unknown / unmapped collection returns null (no surprise cap)', () => {
    ...
    expect(getCollectionDailyCap('com.dinakernel.peerlens.somethingNew')).toBeNull()
  })
})

...
describe('trace_id propagation — TN-OBS-002', () => {
  ...
  it('RecordOp shape carries optional traceId', () => {
    ...
    // nested object).
    const op: RecordOp = {
      uri: 'at://did:plc:author/com.dinakernel.peerlens.attestation/abc',
      did: 'did:plc:author',
  ...
  it('rejection writer log line includes trace_id when ctx supplies one', async () => {
    ...
    await recordRejection(
      ...
      {
        atUri: 'at://did:plc:author/com.dinakernel.peerlens.attestation/x',
        did: 'did:plc:author',
        reason: 'rate_limit',
      },
    )
  ...
  it('rejection writer omits trace_id when ctx does not supply one (legacy path)', async () => {
    ...
    await recordRejection(
      ...
      {
        atUri: 'at://did:plc:author/com.dinakernel.peerlens.attestation/y',
        did: 'did:plc:author',
        reason: 'feature_off',
      },
    )

...
describe('§3.1 AT URI Parser', () => {
  ...
  it('UT-URI-001: parse valid AT URI', () => {
    const result = parseAtUri('at://did:plc:abc/com.dinakernel.peerlens.attestation/tid123')
    expect(result.did).toBe('did:plc:abc')
    expect(result.collection).toBe('com.dinakernel.peerlens.attestation')
  ...
  it('UT-URI-003: construct AT URI', () => {
    const uri = constructAtUri('did:plc:abc', 'com.dinakernel.peerlens.attestation', 'tid123')
    expect(uri).toBe('at://did:plc:abc/com.dinakernel.peerlens.attestation/tid123')
  })
  ...
  it('UT-URI-007: round-trip: parse -> construct -> parse', () => {
    const original = 'at://did:plc:abc/com.dinakernel.peerlens.vouch/tid456'

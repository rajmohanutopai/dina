/**
 * Unit tests for `appview/src/scorer/peerlens-v1-params-reader.ts` (TN-SCORE-009).
 *
 * Contract:
 *   - `readPeerlensV1Params(db)` returns a frozen typed snapshot
...
import {
  readPeerlensV1Params,
  readCachedTrustV1Params,
  clearParamsCache,
} from '@/scorer/peerlens-v1-params-reader'
import { TRUST_V1_PARAM_SEEDS } from '@/db/seeds/peerlens-v1-params'
...
describe('readPeerlensV1Params — TN-SCORE-009', () => {
  it('falls back to seed defaults when DB has no rows', async () => {
    const rows = { value: [] as StubRow[], calls: 0 }
    const params = await readPeerlensV1Params(stubDb(rows))

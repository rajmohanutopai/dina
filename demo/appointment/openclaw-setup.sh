#!/usr/bin/env bash
# OpenClaw setup for the demo appointment provider (Dr Carl).
# Same shape as demo/transit/openclaw-setup.sh, just swaps the transit
# MCP for appointment.
set -euo pipefail

CONFIG_DIR="$HOME/.openclaw"
CONFIG_FILE="${CONFIG_DIR}/openclaw.json"
OC_TOKEN="${OPENCLAW_TOKEN:-drcarl-oc-token-$(date +%s)}"

mkdir -p "$CONFIG_DIR"

echo "==> Writing OpenClaw config (with appointment MCP tool)"
...
if [ -n "${DINA_PAIRING_CODE:-}" ] && [ -n "${DINA_CORE_URL:-}" ]; then
    dina configure --headless \
        ...
        --device-name "drcarl-openclaw" \
        --config-dir /root \
        --role agent
    export DINA_CONFIG_DIR=/root/.dina/cli
    echo "==> dina-agent paired with Dr Carl Core"
else
...
export DINA_HOOK_CALLBACK_TOKEN="${DINA_HOOK_CALLBACK_TOKEN:-dina-callback-drcarl-token}"

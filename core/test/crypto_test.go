...
// 19 scenarios across 8 subsections (BIP-39, SLIP-0010, HKDF, Argon2id,
// Ed25519 Signing, Ed25519→X25519, NaCl crypto_box_seal, AES-256-GCM).
// ==========================================================================

// --------------------------------------------------------------------------
// §2.1 BIP-39 Mnemonic Generation & Recovery (10 scenarios)
// --------------------------------------------------------------------------
...
func TestCrypto_2_1_GenerateMnemonic24Words(t *testing.T) {
	...
	testutil.RequireNotNil(t, seed)

	// Count words: split on spaces, expect exactly 24.
	words := splitWords(mnemonic)
	testutil.RequireLen(t, len(words), 24)

	// Seed must be 512 bits (64 bytes).
	testutil.RequireBytesLen(t, seed, 64)
}
...
func TestCrypto_2_1_MnemonicToSeedKnownVector(t *testing.T) {
	...
	seed, err := impl.ToSeed(testutil.TestMnemonic24, "")
	testutil.RequireNoError(t, err)
	testutil.RequireBytesEqual(t, seed, testutil.TestMnemonicSeed)
}
...
func TestCrypto_2_1_InvalidChecksumRejected(t *testing.T) {
	...
	err := impl.Validate(testutil.TestMnemonicInvalidChecksum)
...
func TestCrypto_2_1_WrongWordCountRejected(t *testing.T) {
	...
	err := impl.Validate(testutil.TestMnemonic12Word)
	testutil.RequireError(t, err)
}
...
func TestCrypto_2_1_ExtraWhitespaceNormalized(t *testing.T) {
	...
	// Extra whitespace should be normalized — validation must pass.
	err := impl.Validate(testutil.TestMnemonicExtraSpaces)
	testutil.RequireNoError(t, err)

	// Seed from extra-space mnemonic should equal seed from canonical mnemonic.
	seedNormal, err := impl.ToSeed(testutil.TestMnemonic24, "")
	testutil.RequireNoError(t, err)
	seedSpaced, err := impl.ToSeed(testutil.TestMnemonicExtraSpaces, "")
	testutil.RequireNoError(t, err)
	testutil.RequireBytesEqual(t, seedNormal, seedSpaced)
}
...
func TestCrypto_2_1_MasterSeedIsDEK(t *testing.T) {
	...
	// BIP-39 produces a 512-bit (64-byte) seed used as master key material.
	testutil.RequireBytesLen(t, seed, 64)
}
...
func TestCrypto_2_1_MnemonicRecoveryDeterministic(t *testing.T) {
	...
	// Same mnemonic must produce the same root keypair every time.
	seed1, err := impl.ToSeed(testutil.TestMnemonic24, "")
	testutil.RequireNoError(t, err)
	seed2, err := impl.ToSeed(testutil.TestMnemonic24, "")
	testutil.RequireNoError(t, err)
	testutil.RequireBytesEqual(t, seed1, seed2)
}
...
func TestCrypto_2_1_RecoverySameIdentity(t *testing.T) {
	...
	// BIP-39 → seed → SLIP-0010 root → DID must be identical across recoveries.
	seed, err := mnemonicImpl.ToSeed(testutil.TestMnemonic24, "")
	testutil.RequireNoError(t, err)
...
func TestCrypto_2_1_LoseDevicePaperIdentityGone(t *testing.T) {
	...
	// Generate a mnemonic, derive seed, then attempt recovery without it.
	_, seed1, err := impl.Generate()
	testutil.RequireNoError(t, err)

	// A different mnemonic produces a completely different seed.
	_, seed2, err := impl.Generate()
	testutil.RequireNoError(t, err)

	// Without the original mnemonic, the original seed is unrecoverable.
	testutil.RequireBytesNotEqual(t, seed1, seed2)
}

// TST-CORE-056, TST-CORE-057, TST-CORE-058, TST-CORE-059, TST-CORE-060, TST-CORE-061, TST-CORE-062
// TST-CORE-063, TST-CORE-064, TST-CORE-065
func TestCrypto_2_1_RootNeverTransmittedPlaintext(t *testing.T) {
	// This is a design audit test: the master seed / root key must never
	// appear in network traffic. Implementation should use in-memory-only
	// handling and never serialize the root seed to any transport layer.
	//
	...
	// Generate a seed and confirm it stays in-memory only.
	// The MnemonicGenerator interface has no method to transmit or export
	// the master seed — this is by design. The contract enforces that the
	// raw seed never leaves process memory.
	_, seed, err := impl.Generate()
	testutil.RequireNoError(t, err)
	testutil.RequireBytesLen(t, seed, 64)

	// Verify the interface has no export/transmit API (compile-time contract).
	// The absence of such methods on MnemonicGenerator IS the assertion.
}
...
// splitWords splits a string by spaces, filtering out empty strings.

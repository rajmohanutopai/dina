...
// ---------- §B4 — BIP-39 Recovery Safety ----------

// TST-ADV-034: BIP-39 mnemonic with invalid checksum is rejected.
// Architecture §6: "mnemonic checksum validation — reject corrupted recovery phrases."
func TestAdv_29_8_BIP39InvalidChecksum(t *testing.T) {
	gen := dinacrypto.NewBIP39Generator()

	// Generate a valid mnemonic.
	mnemonic, _, err := gen.Generate()
	if err != nil {
		t.Fatalf("Generate: %v", err)
	}

	// Validate the original — should pass.
	if err := gen.Validate(mnemonic); err != nil {
		t.Fatalf("valid mnemonic rejected: %v", err)
	}

	// Corrupt the last word (changes checksum).
	...
	corrupted := joinWords(words)

	err = gen.Validate(corrupted)
	if err == nil {
		t.Fatal("corrupted mnemonic (bad checksum) should be rejected")
	}
}

// TST-ADV-035: BIP-39 with wrong word count (12 instead of 24) is rejected.
// Architecture §6: "Only 24-word mnemonics (256-bit entropy) are accepted."
func TestAdv_29_8_BIP39WrongWordCount(t *testing.T) {
	gen := dinacrypto.NewBIP39Generator()

	// 12-word mnemonic — should be rejected (Dina requires 24).
	shortMnemonic := "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about"
	err := gen.Validate(shortMnemonic)
	if err == nil {
		t.Fatal("12-word mnemonic should be rejected — Dina requires 24 words")
	}
}

// TST-ADV-036: Same mnemonic always produces same seed (deterministic recovery).
// Architecture §6: "deterministic derivation — same mnemonic → same seed → same keys."
func TestAdv_29_8_BIP39DeterministicSeed(t *testing.T) {
	gen := dinacrypto.NewBIP39Generator()
	...
	seed1, err := gen.ToSeed(mnemonic, "")
	if err != nil {
		t.Fatalf("ToSeed (1): %v", err)
	}
	seed2, err := gen.ToSeed(mnemonic, "")
	if err != nil {
		t.Fatalf("ToSeed (2): %v", err)
	}

	if !bytes.Equal(seed1, seed2) {
		t.Fatal("same mnemonic must produce identical seeds — recovery depends on determinism")
	}

	// Seed should be 64 bytes (512-bit).
	if len(seed1) != 64 {
		t.Fatalf("seed should be 64 bytes (512-bit), got %d", len(seed1))
	}
}

// splitMnemonicWords splits a mnemonic string into words.
func splitMnemonicWords(mnemonic string) []string {
	var words []string
	start := 0
	for i, c := range mnemonic {
		if c == ' ' {
...
// joinWords concatenates words with spaces (helper for mnemonic manipulation).

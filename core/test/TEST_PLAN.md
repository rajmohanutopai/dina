...
| 12 | **[TST-CORE-161]** Persona Ed25519 key signs PeerLens entries (NOT root key) | Persona publishes attestation | Signed by persona key (e.g., `/consumer` at `m/9999'/1'/0'/0'`), verifiable against persona's DID Document — root key cannot sign on behalf of persona |
...
### 3.4.10 Contact Preferences (`preferred_for`)

> User-asserted "this is my go-to for X" category bindings on contacts.
> Added in migration v15. Replaces the auto-enriched live_capability
> path the Working Memory branch built — that conflated AppView
> capability data (what a DID publishes) with user preference (who
> the user chooses to use). See `docs/WORKING_MEMORY_DESIGN.md` §6.1
> for the shift in reasoning.
>
> Normalisation contract (enforced by both the SQLite-backed and
> in-memory implementations): lowercase, trimmed, deduped, empty
> strings dropped. Input order is preserved so the user's first-named
> preference stays first.

| # | Scenario | Input | Expected |
|---|----------|-------|----------|
| 1 | **[TST-CORE-1000]** Set + Get roundtrip | `SetPreferredFor(did, ["dental"])` then `GetPreferredFor(did)` | Returns `["dental"]`; fresh contact returns empty list (not error) |
| 2 | **[TST-CORE-1001]** Input normalisation | `["  TAX  ", "accounting", "Tax", "", "ACCOUNTING"]` | Stored as `["tax", "accounting"]` — lowercased, trimmed, deduped, empty dropped, first-seen order preserved |
| 3 | **[TST-CORE-1002]** Empty input clears preferences | Set then `SetPreferredFor(did, [])` | `GetPreferredFor` returns empty list; previous preferences removed |
| 4 | **[TST-CORE-1003]** Unknown contact errors | `SetPreferredFor` or `GetPreferredFor` on non-existent DID | Returns `ErrContactNotFound`; no row created implicitly |
| 5 | **[TST-CORE-1004]** `FindByPreferredFor` case-insensitive | Lookup `"Dental"`, `"  DENTAL  "`, `"dental"` | All resolve to the same contact; whitespace + case tolerated |
| 6 | **[TST-CORE-1005]** `FindByPreferredFor` multi-match | Sancho (friend + dentist), Dr Carl (dentist) both tagged `dental`; Mike tagged `plumbing` | Both dentist-tagged contacts returned; Mike excluded. Exercises the multi-faceted-contact case (friend who is also a service provider) |
| 7 | **[TST-CORE-1006]** Empty / whitespace / unknown category returns nothing | `FindByPreferredFor("")`, `"   "`, `"astrology"` | Empty slice, no error — the resolver must always pass a concrete intent; no fuzzy fallback |
| 8 | **[TST-CORE-1007]** `List` includes `PreferredFor` | Set preferences, call `List` | Contact row in list carries the normalised `PreferredFor` slice verbatim |
| 9 | **[TST-CORE-1008]** `PUT /v1/contacts/{did}` accepts `preferred_for` | HTTP PUT body `{"preferred_for": ["dental"]}` | 200 OK; directory's `GetPreferredFor` returns `["dental"]` |
| 10 | **[TST-CORE-1009]** PUT without `preferred_for` key leaves it alone | HTTP PUT body `{"name": "…"}` — `preferred_for` omitted | Name updated; existing preferences untouched. Tri-state semantics: nil = no-op |
| 11 | **[TST-CORE-1010]** PUT with empty `preferred_for` clears | HTTP PUT body `{"preferred_for": []}` | Name preserved; preferences cleared. Tri-state semantics: `[]` = clear |
...
| # | Scenario | Input | Expected |
|---|----------|-------|----------|
| 1 | **[TST-CORE-710]** Sign attestation record | Brain requests `POST /v1/trust/publish` with attestation payload | Core signs with persona key → writes to PDS as `com.dinakernel.peerlens.attestation` record |
...
| 5 | **[TST-CORE-918]** `com.dinakernel.peerlens.bot` and `com.dinakernel.peerlens.membership` Lexicons validated | Bot/membership Lexicon records | Schema validation passes for bot and membership Lexicon types |

...
func (s *stubDIDResolver) Resolve(_ context.Context, _ domain.DID) (*domain.DIDDocument, error) {
	return &domain.DIDDocument{}, nil
}
...
func TestHandleIngestNaCl_NoIngressRouter_DirectPath(t *testing.T) {
	...
	// Build a valid DinaMessage that will pass through the stub decryptor.
	msgJSON, _ := json.Marshal(domain.DinaMessage{
		Type: domain.MessageTypeQuery,
		To:   []string{"did:key:z6MkTest"},
		Body: []byte("hello"),
...
func TestProcessPending_SweepsDeadDrop(t *testing.T) {
	...
	// Store some blobs in the dead drop.
	for i := 0; i < 3; i++ {
		if err := dd.Store(context.Background(), []byte("blob-data")); err != nil {

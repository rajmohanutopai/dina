...
var _ port.PeerlensResolver = (*Resolver)(nil)
...
// SearchPeerlens queries AppView's search endpoint for product/entity PeerLens data.
// Returns the raw JSON response (attestation results).
func (r *Resolver) SearchPeerlens(query, category, subjectType string, limit int) (json.RawMessage, error) {
	if r.baseURL == "" {
		return nil, domain.ErrAppViewNotConfigured
	}
	if limit <= 0 || limit > 25 {

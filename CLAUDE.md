...
- **Plugins are signed contracts, never in-process code.** The plugin substrate (`docs/PLUGIN_ARCHITECTURE.md`, `packages/core/src/plugins/*`, `packages/protocol/src/plugins/*`) preserves the kernel rule: a plugin is a signed, content-addressed manifest that runs *either* as **data** interpreted by a hardened first-party interpreter (a generalized `capability_runtime`), *or* as **code** out-of-process, paired as a device with its own Ed25519 key on a private lane (`plugin:<install_id>`). Plugins are **not agents** (a plugin is a bounded capability installed *into* Dina; an agent is an external reasoner acting *under* Dina's authority). Substrate status (2026-09-13): the third-party install path — repo-proof verifier (one shared chain, `@dina/home-node/repo_proof_chain`; server `@dina/net-node`, phone `@dina/net-expo/repo_proof` bundled statically with Metro shims, wired only after an on-device self-check over a shipped fixture repo passes), `/v1/plugins/install/*` routes, the phone's ceremony + consent card + Plugins screen, runner pairing bound in Core, teardown + abandoned-install sweep — is built and tested on both hosts; the phone's chain is bundle-proven (`expo export`), a device run against a live PDS is still owed. First-party manifests (commerce packs, country packs) enter through their own door (`beginFirstPartyInstall`, `local_publisher_key`). The owner-side dispatch producer exists (`plugins/invoke.ts`: one task on the plugin lane, `queued` under a grant or `pending_approval` for the owner's inbox card; `POST /v1/plugins/invoke`). `/ask` reaches installed plugins through Brain's `invoke_plugin` tool (`/v1/plugins/tool-invoke`, the same gate). A completed result renders through the capability's own pinned card template (§15.6, 2026-09-15): the manifest owns the layout, a slot names a `result_schema` field, and the render keeps only blocks — no badges, no links, no plugin-authored frame; a capability with no template still answers on the `label: value` floor. The Core-owned context projector is built (2026-09-15): a caller names a SUBJECT, never a context, and Core projects the facts through a first-party versioned template table (`plugins/context_templates.ts`), with the §11 persona ring now derived rather than left to a policy nobody passed. Still P0: no advisory worker, no plugin SDK workflow.
